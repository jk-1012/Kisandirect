...
describe('Escrow state machine and payments (Jest + supertest)', () => {
    ...
    test('concurrent orders and race conditions cause consistent escrow states', async () => {
        ...
        const orderPromises = Array.from({ length: 10 }).map(() => request(app.server).post('/orders').send({ buyerId: 'b', items: [{ listingId, qty: 1, pricePaise: 10000 }], totalAmountPaise: 10000 }));
