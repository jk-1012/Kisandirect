...
import { createDigiLockerService } from '../services/digilocker-service';
describe('DigiLocker KYC Integration Tests', () => {
    ...
    describe('KYC Initiation', () => {
        ...
        it('should POST /farmers/kyc/initiate return auth URL', async () => {
            ...
            await server.db.query('INSERT INTO farmers (id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [testFarmerId, testUserId]);
    ...
    describe('KYC Status', () => {
        it('should retrieve KYC session status', async () => {
            ...
            await server.queues.connection.setEx(`kyc:session:${sessionId}`, 3600, JSON.stringify(mockSession));
            const response = await server.inject({
                method: 'GET',
    ...
    describe('Document Storage', () => {
        it('should retrieve KYC document reference number', async () => {
            ...
            await server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, aadhaar_ref_number, updated_at)
         VALUES ($1, $2, NOW())
         ON CONFLICT (farmer_id) DO UPDATE SET aadhaar_ref_number = $2`, [testFarmerId, 'test-ref-123']);
            const response = await server.inject({
                method: 'GET',
        ...
        it('should return 404 when document not found', async () => {
            ...
            await server.db.query('INSERT INTO farmers (id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [newFarmerId, testUserId]);
            const response = await server.inject({
                method: 'GET',
                url: '/api/v1/farmers/kyc/document/AADHAAR',
                headers: {
    ...
    describe('Current User KYC Status', () => {
        it('should return current user KYC status', async () => {
            ...
            await server.db.query(`INSERT INTO farmers (id, user_id, kyc_status, kyc_verified_at, kyc_expires_at)
         VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '365 days')
         ON CONFLICT DO NOTHING`, [freshFarmerId, freshUserId, 'VERIFIED']);
            const response = await server.inject({
                method: 'GET',
        ...
        it('should detect expired KYC', async () => {
         ...
         ON CONFLICT DO NOTHING`, [expiredFarmerId, expiredUserId, 'EXPIRED']);
            const response = await server.inject({
                method: 'GET',
                url: '/api/v1/farmers/kyc/me',
                headers: {
    ...
    describe('Audit Logging', () => {
        it('should log KYC events to audit table', async () => {
            ...
            await server.db.query(`INSERT INTO vault.kyc_audit_logs 
         (event_id, event_type, ip_address, user_agent, details, status)
         VALUES ($1, $2, $3, $4, $5, $6)`, [eventId, 'DOCUMENT_FETCHED', '127.0.0.1', 'test-agent', JSON.stringify({ test: true }), 'success']);
            const result = await server.db.query(`SELECT * FROM vault.kyc_audit_logs WHERE event_id = $1`, [eventId]);
...
async function createTestToken(server, userId) {
