...
export class VaultEncryptionService {
    constructor(masterKeyHex, keyId = 'default') {
...
export class PostgresVaultService {
     ...
     */
    async encryptWithPgCrypto(plaintext) {
        const result = await this.server.db.query(`SELECT encode(encrypt($1, $2, 'aes'), 'hex') as encrypted`, [plaintext, this.encryptionKey]);
        return result.rows[0].encrypted;
    }
    /**
     * Decrypt data stored with PostgreSQL pgcrypto
     */
    async decryptWithPgCrypto(encrypted) {
        const result = await this.server.db.query(`SELECT decrypt(decode($1, 'hex'), $2, 'aes') as decrypted`, [encrypted, this.encryptionKey]);
     ...
     */
    async storeAadhaarEncrypted(farmerId, aadhaarNumber, aadhaarRefNumber) {
        await this.server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, aadhaar_encrypted, aadhaar_ref_number, kyc_updated_at)
       VALUES ($1, encode(encrypt($2, $3, 'aes'), 'hex'), $4, NOW())
       ON CONFLICT (farmer_id) DO UPDATE SET 
         aadhaar_encrypted = encode(encrypt($2, $3, 'aes'), 'hex'),
         aadhaar_ref_number = $4,
         kyc_updated_at = NOW()`, [farmerId, aadhaarNumber, this.encryptionKey, aadhaarRefNumber]);
    }
    /**
     * Store encrypted PAN data in vault
     */
    async storePANEncrypted(farmerId, panNumber, panRefNumber) {
        await this.server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, pan_encrypted, pan_ref_number, kyc_updated_at)
       VALUES ($1, encode(encrypt($2, $3, 'aes'), 'hex'), $4, NOW())
       ON CONFLICT (farmer_id) DO UPDATE SET 
         pan_encrypted = encode(encrypt($2, $3, 'aes'), 'hex'),
         pan_ref_number = $4,
         kyc_updated_at = NOW()`, [farmerId, panNumber, this.encryptionKey, panRefNumber]);
    }
    /**
     * Store encrypted land ownership data
     */
    async storeLandOwnershipEncrypted(farmerId, landData, refNumber) {
        await this.server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, land_ownership_encrypted, land_ownership_ref_number, kyc_updated_at)
       VALUES ($1, encode(encrypt($2, $3, 'aes'), 'hex'), $4, NOW())
       ON CONFLICT (farmer_id) DO UPDATE SET 
         land_ownership_encrypted = encode(encrypt($2, $3, 'aes'), 'hex'),
         land_ownership_ref_number = $4,
         kyc_updated_at = NOW()`, [farmerId, landData, this.encryptionKey, refNumber]);
    }
    /**
     * Retrieve encrypted Aadhaar (must be decrypted by application)
     */
    async getAadhaarEncrypted(farmerId) {
        const result = await this.server.db.query(`SELECT aadhaar_encrypted FROM vault.farmer_kyc WHERE farmer_id = $1`, [farmerId]);
        return result.rows[0]?.aadhaar_encrypted || null;
    }
    /**
     * Retrieve and decrypt Aadhaar (use with caution - only when needed)
     */
    async getAadhaarDecrypted(farmerId) {
        const result = await this.server.db.query(`SELECT decrypt(decode(aadhaar_encrypted, 'hex'), $1, 'aes') as aadhaar 
       FROM vault.farmer_kyc WHERE farmer_id = $2`, [this.encryptionKey, farmerId]);
        if (!result.rows[0]?.aadhaar)
            return null;
     ...
     */
    async getPANDecrypted(farmerId) {
        const result = await this.server.db.query(`SELECT decrypt(decode(pan_encrypted, 'hex'), $1, 'aes') as pan 
       FROM vault.farmer_kyc WHERE farmer_id = $2`, [this.encryptionKey, farmerId]);
        if (!result.rows[0]?.pan)
            return null;
     ...
     */
    async markKYCCompleted(farmerId, kycStatus, digilockerRef) {
        await this.server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, kyc_status, digilocker_ref, kyc_completed_at, kyc_expires_at)
       VALUES ($1, $2, $3, NOW(), NOW() + INTERVAL '365 days')
       ON CONFLICT (farmer_id) DO UPDATE SET 
         kyc_status = $2,
         digilocker_ref = $3,
         kyc_completed_at = NOW(),
         kyc_expires_at = NOW() + INTERVAL '365 days'`, [farmerId, kycStatus, digilockerRef]);
    }
}
/**
