...
export async function processReleaseEscrow(server, jobData) {
    ...
    await server.db.query(`UPDATE public.farmer_profiles SET annual_payout_inr = COALESCE(annual_payout_inr,0) + $1, tds_deducted_inr = COALESCE(tds_deducted_inr,0) + $2 WHERE user_id = $3`, [Math.round(subtotalPaise / 100), Math.round(tdsPaise / 100), order.farmer_id]);
    ...
    await server.db.query(`INSERT INTO audit.transaction_ledger(txn_id, order_id, event_type, amount_paise, farmer_id, buyer_id, metadata, prev_hash, entry_hash, created_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW())`, [`TXN-${crypto.randomBytes(6).toString('hex')}`, order.id, 'ESCROW_RELEASED', payoutPaise, order.farmer_id, order.buyer_id, { payout_id: payoutId, tds_paise: tdsPaise }, prevHash, entryHash]);
