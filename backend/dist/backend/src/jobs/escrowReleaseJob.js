...
export async function processReleaseEscrow(server, jobData) {
    const { orderId } = jobData;
    const res = await server.db.query(`SELECT o.*, fp.annual_payout_inr, fp.tds_deducted_inr, vk.bank_account_token
    ...
    const { tds_paise } = await createTDSService(server).calculateTDS(order.farmer_id, subtotalPaise);
    const payoutPaise = subtotalPaise - tds_paise;
    ...
    const payoutId = payoutJson.id;
    await server.db.query(`UPDATE public.orders SET order_status = $1, escrow_release_at = NOW() WHERE order_id = $2`, ['RELEASED', orderId]);
    await server.db.query(`UPDATE public.farmer_profiles SET annual_payout_inr = COALESCE(annual_payout_inr,0) + $1, tds_deducted_inr = COALESCE(tds_deducted_inr,0) + $2 WHERE user_id = $3`, [Math.round(subtotalPaise / 100), Math.round(tds_paise / 100), order.farmer_id]);
    ...
    const entryHash = crypto.createHash('sha256').update(prevHash + JSON.stringify({ orderId, payoutPaise, tds_paise, payoutId }) + now).digest('hex');
    await server.db.query(`INSERT INTO audit.transaction_ledger(txn_id, order_id, event_type, amount_paise, farmer_id, buyer_id, metadata, prev_hash, entry_hash, created_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW())`, [`TXN-${crypto.randomBytes(6).toString('hex')}`, order.id, 'ESCROW_RELEASED', payoutPaise, order.farmer_id, order.buyer_id, { payout_id: payoutId, tds_paise }, prevHash, entryHash]);
    server.log.info({ orderId, payoutPaise, tds_paise, payoutId }, 'escrow released to farmer');
    return { ok: true, payout_id: payoutId, tds_paise };
}
