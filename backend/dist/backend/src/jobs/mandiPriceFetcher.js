...
async function withRetry(fn, retries = 3, backoff = [1000, 2000, 4000]) {
    let attempt = 0;
    let lastError;
...
async function alertOpsTeam(server, message) {
...
export async function runMandiPriceFetcher(server) {
    ...
    for (const commodity of TARGET_COMMODITIES) {
        for (const state of TARGET_STATES) {
            try {
                ...
                const data = await withRetry(() => fetch(url.toString()).then((r) => r.json()), 3, [1000, 2000, 4000]);
                ...
                for (const record of records) {
                    ...
                    await server.db.query(`INSERT INTO public.mandi_prices
             (commodity, mandi_id, mandi_name, state_code, district, price_date, modal_price_paise, min_price_paise, max_price_paise)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
             ON CONFLICT ON CONSTRAINT mandi_prices_unique_record DO NOTHING`, [commodityKey, mandiId, mandiName, stateCode, district, priceDate, modalPricePaise, minPricePaise, maxPricePaise]);
