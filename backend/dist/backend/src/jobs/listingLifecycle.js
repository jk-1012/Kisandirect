...
async function sendWhatsAppTemplate(server, payload) {
...
async function getListingWithFarmer(server, listingId) {
    const result = await server.db.query(`SELECT l.listing_id, l.crop_type, l.crop_category, l.asking_price_paise, l.status,
...
async function getListingById(server, listingId) {
    const result = await server.db.query('SELECT * FROM public.listings WHERE listing_id = $1', [listingId]);
    return result.rows[0] ?? null;
}
export async function expiryWarning24h(server, listingId) {
...
export async function expiryWarning1h(server, listingId) {
    const listing = await getListingWithFarmer(server, listingId);
    if (!listing || listing.status !== 'ACTIVE') {
        return null;
    }
...
export async function expireListing(server, listingId) {
    const listing = await getListingById(server, listingId);
    if (!listing || listing.status !== 'ACTIVE') {
        return null;
    }
    await server.db.query('UPDATE public.listings SET status = $1, updated_at = NOW() WHERE listing_id = $2', ['EXPIRED', listingId]);
    try {
...
export async function scheduleListingLifecycleJobs(server, listingId) {
...
export async function removeListingLifecycleJobs(server, jobIds) {
    const removeCalls = [];
