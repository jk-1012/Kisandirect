 ...
 */
export async function aggregateFarmerMetrics(db, farmerId) {
    // Get completed orders and ratings
    const orderStats = await db.query(`SELECT
    ...
    FROM public.orders
    WHERE farmer_id = $1 AND created_at >= CURRENT_DATE - INTERVAL '1 year'`, [farmerId]);
    const oStats = orderStats.rows[0];
    // Get disputes for this farmer
    const disputeStats = await db.query(`SELECT
    ...
    // Get KYC level
    const kycInfo = await db.query(`SELECT kyc_level, profile_completeness_percentage
    FROM vault.farmer_kyc
    WHERE farmer_id = $1`, [farmerId]);
    const kyc = kycInfo.rows[0];
    // Get last activity date
    const lastActivityQuery = await db.query(`SELECT MAX(GREATEST(
    ...
    return {
        ...
        cancellationRate: oStats.total_orders > 0
 ...
 * Update queue status
 */
async function updateQueueStatus(db, queueId, status, errorMessage) {
    if (status === 'FAILED' && errorMessage) {
        // Schedule retry
        const retrySchedule = [60000, 300000, 900000]; // 1min, 5min, 15min
        const retryAttempt = await db.query(`SELECT attempt_count FROM vault.farmer_trust_score_recalculation_queue WHERE id = $1`, [queueId]);
        const attempt = retryAttempt.rows[0]?.attempt_count || 0;
        const nextRetryMs = retrySchedule[Math.min(attempt, retrySchedule.length - 1)];
        await db.query(`UPDATE vault.farmer_trust_score_recalculation_queue
      SET status = $1, attempt_count = attempt_count + 1,
          last_error_message = $2, next_retry_at = NOW() + INTERVAL '1 millisecond' * $3
      WHERE id = $4`, [status, errorMessage, nextRetryMs, queueId]);
    }
    else {
        await db.query(`UPDATE vault.farmer_trust_score_recalculation_queue
      ...
      WHERE id = $2`, [status, queueId]);
    }
}
/**
 * Process single farmer trust score recalculation
 */
export async function processTrustScoreRecalculation(server, trustScoreService, jobData, job) {
    try {
        ...
        server.log.info(`[TrustScore] Recalculation completed for farmer ${farmerId}, score: ${result.trustScore.trustScoreNumeric.toFixed(2)}, category: ${result.trustScore.trustScoreCategory}`);
 ...
 */
export async function processBatchTrustScoreRecalculation(server, trustScoreService, jobData, job) {
    try {
        ...
        const farmerIds = result.rows.map((r) => r.farmer_id);
        ...
        for (const farmerId of farmerIds) {
            try {
                ...
                const existing = await db.query(`SELECT id FROM vault.farmer_trust_score_recalculation_queue WHERE farmer_id = $1 AND status = 'PENDING'`, [farmerId]);
                if (existing.rows.length === 0) {
                    // Queue for recalculation
                    await queue.add('RECALCULATE_TRUST_SCORE', {
                        farmerId,
 ...
 */
export async function processPeriodicTrustScoreRecalculation(server, trustScoreService, jobData, job) {
    try {
 ...
 */
export async function queueTrustScoreRecalculation(server, farmerId, reason, priority = 5) {
    const db = server.db;
    const queue = server.queues.payoutQueue;
    try {
        // Queue in database for tracking
        const result = await db.query(`INSERT INTO vault.farmer_trust_score_recalculation_queue (
      ...
      ON CONFLICT (farmer_id) DO UPDATE SET
        ...
        status = 'PENDING',
        updated_at = NOW()
      RETURNING id`, [farmerId, reason, priority]);
