 ...
 */
export async function notificationFallbackHandler(job, server) {
    const { notificationId, userId, type, title, message, channels, attempt, primaryChannelFailed, } = job.data;
    try {
        server.log.info({
            jobId: job.id,
            notificationId,
        ...
        if (fallbackChannels.length === 0) {
            server.log.warn({ jobId: job.id, notificationId, userId }, 'No fallback channels available');
            // Mark notification as permanently failed
            await server.db.query(`UPDATE notifications 
        ...
        for (const channel of fallbackChannels) {
            try {
                server.log.info({ jobId: job.id, notificationId, channel, attempt }, `Attempting to deliver via ${channel}`);
                const result = await deliverNotificationViChannel(server, userId, channel, title, message);
                if (result.success) {
                ...
                server.log.warn({ jobId: job.id, notificationId, channel, error: channelError?.message }, `Failed to deliver via ${channel}`);
       ...
       WHERE id = $1`, [notificationId, successChannel]);
        server.log.info({
            jobId: job.id,
            notificationId,
            userId,
 ...
 */
async function deliverNotificationViChannel(server, userId, channel, title, message) {
    try {
        // Get user contact info
        const userResult = await server.db.query('SELECT email, phone FROM public.users WHERE id = $1', [userId]);
        ...
        server.log.debug({ error: error?.message, channel }, 'Channel delivery attempt failed');
 ...
 */
async function sendEmail(server, email, title, message) {
 ...
 */
async function sendSms(server, phone, message) {
 ...
 */
async function sendPushNotification(server, userId, title, message) {
