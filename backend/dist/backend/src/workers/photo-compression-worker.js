...
export async function photoCompressionHandler(job, server) {
    const { photoKey, bucketName, listingId, farmerUserId, fileName } = job.data;
    try {
        server.log.info({ jobId: job.id, photoKey, bucketName, fileName }, 'Starting photo compression job');
        ...
        await Promise.all([
            server.storage.s3Client.send(new PutObjectCommand({
                Bucket: bucketName,
                Key: thumbKey,
                Body: thumbBuffer,
        ...
        server.log.info({ jobId: job.id, thumbKey, mediumKey }, 'Photo resized and uploaded successfully');
                ...
                try {
                    ...
                    if (visionResponse.ok) {
                        ...
                        if (labels?.length) {
                            ...
                            const shouldFlagImage = labels.some((label) => labelTriggersModeration(label.description));
                            if (shouldFlagImage) {
                        ...
                        if (safeSearch) {
                            ...
                            for (const [key, value] of Object.entries(safeSearch)) {
                                if (SAFE_FLAGGED.has(String(value))) {
                                    moderationResult.flagged = true;
                                    server.log.warn({ jobId: job.id, photoKey, safeSearchKey: key, flagValue: value }, 'Photo flagged for moderation (safe search)');
                                }
                            }
                        }
                        if (moderationResult.flagged) {
                            server.log.warn({ jobId: job.id, photoKey, listingId, labels: moderationResult.labels }, 'Photo flagged for content moderation');
                        }
                    }
                        ...
                        server.log.warn({ jobId: job.id, status: visionResponse.status }, 'Vision API error');
                    }
                }
                catch (visionError) {
                    if (visionError?.name === 'AbortError') {
                        ...
                        server.log.warn({ jobId: job.id, photoKey, error: visionError?.message }, 'Vision API request failed');
                    }
                }
            }
        }
        catch (error) {
            server.log.warn({ jobId: job.id, photoKey, error: error?.message }, 'Vision API moderation failed - continuing');
        ...
        server.log.info({ jobId: job.id, result }, 'Photo compression job completed successfully');
        return result;
    }
    catch (error) {
        server.log.error({
