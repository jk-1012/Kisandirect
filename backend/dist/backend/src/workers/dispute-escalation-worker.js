 ...
 */
export async function disputeEscalationHandler(job, server) {
    ...
    try {
        ...
        const disputeResult = await server.db.query('SELECT id, status, escalation_level, buyer_id, seller_id FROM disputes WHERE id = $1', [disputeId]);
        ...
        if (currentLevel >= newLevel) {
            server.log.info({ jobId: job.id, disputeId, currentLevel, newLevel }, 'Dispute already at or above escalation level');
            return {
                disputeId,
        ...
        if (newLevel >= 2) {
            ...
            const agentResult = await server.db.query(`SELECT id FROM dispute_agents 
         WHERE status = 'ACTIVE' 
         ORDER BY RANDOM() 
         LIMIT 1`);
        ...
        // Update the dispute
        const updateResult = await server.db.query(`UPDATE disputes 
       SET escalation_level = $1, 
           ...
           updated_at = NOW()
       WHERE id = $4 
       RETURNING escalation_level, escalated_at`, [newLevel, assignedAgent, reason, disputeId]);
        ...
        try {
            ...
            server.log.warn({ jobId: job.id, disputeId, error: notificationError?.message }, 'Failed to send escalation notifications');
        }
        server.log.info({
            jobId: job.id,
            disputeId,
