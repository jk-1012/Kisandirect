 ...
 */
export async function listingExpiryHandler(job, server) {
    const { listingId, sellerId, reason } = job.data;
    try {
        server.log.info({ jobId: job.id, listingId, sellerId, reason }, 'Starting listing expiry job');
        // Get current listing
        ...
        if (listing.status !== 'ACTIVE') {
            server.log.info({ jobId: job.id, listingId, currentStatus: listing.status }, 'Listing already expired or archived');
            return {
                listingId,
        ...
        // Archive the listing
        const updateResult = await server.db.query(`UPDATE listings 
       SET status = $1, expired_at = NOW(), expiry_reason = $2, updated_at = NOW() 
       WHERE id = $3 AND seller_id = $4 
       RETURNING status, expired_at`, ['ARCHIVED', reason, listingId, sellerId]);
        ...
        try {
            if (server.sendEvent) {
                await server.sendEvent?.('listing:expired', {
                    listingId,
                    sellerId,
                    reason,
            ...
            server.log.warn({ jobId: job.id, listingId, error: eventError?.message }, 'Failed to publish listing expired event');
        }
        return {
            listingId,
            status: 'ARCHIVED',
