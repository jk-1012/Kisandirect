...
async function translateText(server, text, targetLanguage) {
...
async function sendWhatsAppMessage(server, phone, message, language, template, templateParameters) {
...
async function sendSmsMessage(server, phone, message) {
...
export function createNotificationService(server) {
    async function createNotification(payload) {
        ...
        const templateParameters = payload.templateParameters
            ...
            : null;
        const insertResult = await server.db.query(`INSERT INTO public.notifications (user_id, type, title, body, data, channel, status, sent_at, created_at, updated_at)
    ...
    async function deliverNotification(notificationId) {
        const result = await server.db.query(`SELECT n.*, u.phone AS phone, u.language AS language
       ...
       WHERE n.id = $1`, [notificationId]);
        ...
        const templateParameters = Array.isArray(notification.data?.templateParameters)
            ? notification.data?.templateParameters
            : undefined;
        const whatsappResult = await sendWhatsAppMessage(server, phone, notification.body, language, template, templateParameters);
        if (whatsappResult) {
            await server.db.query(`UPDATE public.notifications SET status = $1, delivered_via = $2, sent_at = NOW(), updated_at = NOW() WHERE id = $3`, ['SENT', 'whatsapp', notificationId]);
        ...
        await server.db.query(`UPDATE public.notifications SET status = $1, updated_at = NOW() WHERE id = $2`, ['FAILED', notificationId]);
        await server.queues.notificationQueue.add(NOTIFICATION_FALLBACK, { notificationId }, { delay: 2 * 60 * 1000, removeOnComplete: true, removeOnFail: false });
        return { notificationId, delivered_via: null };
    }
    async function fallbackNotification(notificationId) {
        const result = await server.db.query(`SELECT n.*, u.phone AS phone, u.language AS language
       FROM public.notifications n
       JOIN public.users u ON n.user_id = u.id
       WHERE n.id = $1`, [notificationId]);
        ...
        if (smsResult) {
            await server.db.query(`UPDATE public.notifications SET status = $1, delivered_via = $2, sent_at = NOW(), updated_at = NOW() WHERE id = $3`, ['SENT', 'sms', notificationId]);
            return { notificationId, delivered_via: 'sms' };
        }
        await server.db.query(`UPDATE public.notifications SET status = $1, updated_at = NOW() WHERE id = $2`, ['FAILED', notificationId]);
        return { notificationId, delivered_via: null };
    }
    async function createInAppNotification(userId, data) {
        return createNotification({ ...data, userId, sendExternal: false, channel: 'in_app' });
    }
    async function getNotifications(userId) {
        const result = await server.db.query(`SELECT id, type, title, body, data, channel, delivered_via, status, is_read, sent_at, created_at
    ...
    async function markAsRead(userId, notificationId) {
        const result = await server.db.query(`UPDATE public.notifications SET is_read = true, updated_at = NOW() WHERE id = $1 AND user_id = $2 RETURNING id`, [notificationId, userId]);
