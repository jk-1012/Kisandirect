    ...
    it('writes ledger entry and verifies integrity', async () => {
        ...
        const fakeClient = {
            query: jest.fn((text, values) => {
                ...
                if (text.includes('SELECT * FROM audit.transaction_ledger'))
                    return Promise.resolve({ rows });
                return Promise.resolve({ rows: [] });
            })
        };
        const fakeServer = { db: { query: fakeClient.query }, log: { warn: jest.fn() } };
        const svc = createLedgerService(fakeServer);
