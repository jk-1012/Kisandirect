export function createOfferService(server) {
    ...
    async function createOffer(buyerId, listingId, quantityKg, offerPricePerKgInr) {
        ...
        await server.db.query(`INSERT INTO public.offers (offer_id, listing_id, buyer_id, farmer_id, quantity_kg, offered_price_paise, status, expires_at, created_at, updated_at)
       VALUES ($1,$2,$3,$4,$5,$6,'PENDING',$7,NOW(),NOW())`, [offerId, listing.id, buyerId, listing.farmer_id, quantityKg, offeredPaise, expiresAt]);
        try {
    ...
    async function createOrderFromOffer(offerRow) {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'PENDING','PENDING','MAKE_OFFER') RETURNING *`, [orderId, offerRow.listing_id, offerRow.farmer_id, offerRow.buyer_id, offerRow.quantity_kg, agreedPricePaise, subtotalPaise, platformFeePaise, totalPaise]);
        const razorpayOrder = await (async () => {
    ...
    async function respondToOffer(farmerId, offerId, action, opts) {
        const offerRes = await server.db.query('SELECT * FROM public.offers WHERE offer_id = $1', [offerId]);
        const offer = offerRes.rows[0];
        if (!offer)
            throw server.httpErrors.notFound('Offer not found');
        if (String(offer.farmer_id) !== String(farmerId))
            throw server.httpErrors.forbidden('Not authorized');
        if (!['PENDING', 'COUNTER_OFFERED'].includes(offer.status))
            throw server.httpErrors.badRequest('Offer cannot be acted upon');
        if (action === 'ACCEPT') {
        ...
        if (action === 'COUNTER') {
            if (!opts?.counter_price_per_kg_inr)
                throw server.httpErrors.badRequest('counter_price_per_kg_inr required for counter');
    ...
    async function expireOfferHandler(offerId) {
