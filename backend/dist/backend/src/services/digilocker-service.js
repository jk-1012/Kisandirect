...
export class DigiLockerService {
    constructor(server, config) {
        ...
        this.encryptionService = new VaultEncryptionService(process.env.ENCRYPTION_KEY || '', process.env.ENCRYPTION_KEY_ID || 'default');
    }
    /**
     * Generate DigiLocker authorization URL for OAuth2 flow
     * Creates state token for CSRF protection and stores in Redis
     */
    async generateAuthorizationUrl(userId, farmerId) {
     ...
     */
    async fetchAadhaarData(accessToken, retryCount = 0) {
        try {
            ...
            return this.handleRetry('fetchAadhaarData', error, retryCount, () => this.fetchAadhaarData(accessToken, retryCount + 1));
        }
    }
    /**
     * Fetch PAN data from DigiLocker with retry logic
     */
    async fetchPANData(accessToken, retryCount = 0) {
        try {
            ...
            return this.handleRetry('fetchPANData', error, retryCount, () => this.fetchPANData(accessToken, retryCount + 1));
        }
    }
    /**
     * Fetch Land Ownership data from DigiLocker with retry logic
     */
    async fetchLandOwnershipData(accessToken, retryCount = 0) {
        try {
            ...
            return this.handleRetry('fetchLandOwnershipData', error, retryCount, () => this.fetchLandOwnershipData(accessToken, retryCount + 1));
        }
    }
    /**
     ...
     */
    async encryptAndStoreKYC(farmerId, aadhaarData, panData, landOwnershipData, digilockerRef) {
        ...
        const landOwnershipEncrypted = this.encryptionService.encrypt(JSON.stringify(landOwnershipData));
        // Store in PostgreSQL vault with encrypted data
        await this.vaultService.storeAadhaarEncrypted(farmerId, aadhaarData.uid, aadhaarData.refNumber);
        await this.vaultService.storePANEncrypted(farmerId, panData.number, panData.refNumber);
        await this.vaultService.storeLandOwnershipEncrypted(farmerId, JSON.stringify(landOwnershipData), landOwnershipData.refNumber);
    ...
    async handleCallback(code, state) {
        ...
        try {
            ...
            const sessionData = await this.server.queues.connection.get(sessionKey);
            if (!sessionData)
                throw new Error('Session context lost');
            ...
            await this.server.queues.connection.setEx(`kyc:session:${sessionId}`, ttl, JSON.stringify(kycSession));
            ...
            const vaultRecord = await this.encryptAndStoreKYC(farmerId, aadhaarData, panData, landOwnershipData, `${userId}-${Date.now()}`);
            ...
            this.server.log.info({ userId, farmerId, sessionId, duration, documentsCount: kycSession.documentsFetched?.length || 0 }, 'KYC flow completed successfully');
            return kycSession;
        }
        catch (error) {
     ...
     */
    async handleRetry(operationName, error, retryCount, retryFn) {
    ...
    async auditLog(eventType, details) {
        ...
        try {
            await this.server.db.query(`INSERT INTO vault.kyc_audit_logs 
 ...
 */
export function createDigiLockerService(server) {
    const config = {
