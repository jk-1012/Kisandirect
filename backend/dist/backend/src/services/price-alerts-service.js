export function createPriceAlertService(server) {
    /**
     * Create or update a price alert for a farmer
     * Farmer receives WhatsApp notification when market price crosses threshold
     */
    async function createPriceAlert(farmerId, payload) {
        // Validate farmer exists and is verified
        const farmerRes = await server.db.query('SELECT id FROM public.users WHERE id = $1 AND role = $2 AND kyc_status = $3', [farmerId, 'FARMER', 'VERIFIED']);
        if (!farmerRes.rows[0]) {
        ...
        const existingRes = await server.db.query(`SELECT id FROM public.price_alerts
       WHERE farmer_id = $1 AND crop_type = $2 AND state_code = $3 AND direction = $4 AND active = TRUE`, [farmerId, payload.crop_type, payload.state_code, payload.direction]);
        if (existingRes.rows[0]) {
            throw server.httpErrors.conflict(`Active alert already exists for ${payload.crop_type} in ${payload.state_code} (${payload.direction})`);
        }
        // Create or update alert
        const result = await server.db.query(`INSERT INTO public.price_alerts
       ...
       RETURNING id`, [farmerId, payload.crop_type, payload.state_code, payload.threshold_price_per_kg_inr, payload.direction]);
        server.log.info({
            farmer_id: farmerId,
     ...
     */
    async function deletePriceAlert(farmerId, alertId) {
        const result = await server.db.query(`UPDATE public.price_alerts
       SET active = FALSE, updated_at = NOW()
       WHERE id = $1 AND farmer_id = $2
       RETURNING crop_type, state_code, direction`, [alertId, farmerId]);
        if (!result.rows[0]) {
            throw server.httpErrors.notFound('Alert not found or not authorized');
        }
        const alert = result.rows[0];
        server.log.info({ farmer_id: farmerId, crop_type: alert.crop_type, direction: alert.direction }, 'Price alert deactivated');
        return { success: true, message: 'Alert deactivated' };
    }
    /**
     * Get farmer's active alerts
     */
    async function getFarmerAlerts(farmerId) {
        const result = await server.db.query(`SELECT id, crop_type, state_code, threshold_price_per_kg_inr, direction, last_triggered_at, created_at
       FROM public.price_alerts
       WHERE farmer_id = $1 AND active = TRUE
       ORDER BY crop_type, state_code, direction`, [farmerId]);
        return {
    ...
    async function checkAndNotifyPriceAlerts() {
        ...
        const alertsRes = await server.db.query(`SELECT pa.*, u.phone, u.language FROM public.price_alerts pa
       ...
       ORDER BY pa.crop_type, pa.state_code`, []);
        ...
        for (const [key, alerts] of alertsByKey) {
            ...
            try {
                ...
                for (const alert of alerts) {
                    ...
                    if (shouldTrigger) {
                        try {
                            await notifyFarmerOfPrice(alert.farmer_id, alert.phone, alert.language, cropType, stateCode, currentPrice, alert.direction);
                            // Update last_triggered_at
                            await server.db.query('UPDATE public.price_alerts SET last_triggered_at = NOW() WHERE id = $1', [alert.id]);
                            triggered += 1;
                        }
                        catch (err) {
        ...
        server.log.info({ checked: alertsRes.rows.length, triggered, errors }, 'Price alert check completed');
     ...
     */
    async function fetchMarketPrice(cropType, stateCode) {
        try {
     ...
     */
    async function notifyFarmerOfPrice(farmerId, phone, language, cropType, stateCode, currentPrice, direction) {
        ...
        try {
            const notifyRes = await server.db.query(`INSERT INTO public.notifications (user_id, type, title, body, data, channel, status, created_at, updated_at)
         ...
         RETURNING id`, [
            ...
            if (notificationId) {
                ...
                await server.queues.notificationQueue.add('DELIVER_NOTIFICATION', { notificationId }, { attempts: 3, backoff: { type: 'exponential', delay: 5000 } });
            }
        }
        catch (err) {
     ...
     */
    async function getPriceHistory(cropType, stateCode, days = 30) {
        ...
        const result = await server.db.query(`SELECT date_trunc('day', price_date) as date, AVG(price_inr_per_kg) as avg_price, 
       ...
       ORDER BY date DESC`, [cropType, stateCode, days]);
        return {
            crop_type: cropType,
