...
export function createComplianceService(server) {
    async function createConsent(userId, consentType, consented, policyVersion, ipAddress) {
        const result = await server.db.query(`INSERT INTO public.consent_records (user_id, consent_type, consented, policy_version, consented_at, ip_address)
       VALUES ($1,$2,$3,$4,NOW(),$5)
       RETURNING id`, [userId, consentType, consented, policyVersion, ipAddress ?? null]);
    ...
    async function listConsents(userId) {
        const result = await server.db.query(`SELECT consent_type, consented, policy_version, consented_at, ip_address
       FROM public.consent_records
       WHERE user_id = $1
       ORDER BY consented_at DESC`, [userId]);
        return result.rows;
    }
    async function submitDataAccessRequest(userId, note) {
        const requestId = generateComplianceRequestId();
        const requestData = { note, requestor: userId };
        await server.db.query(`INSERT INTO public.data_access_requests (request_id, user_id, request_type, status, requested_at, request_data)
       VALUES ($1,$2,'ACCESS','PENDING',NOW(),$3)`, [requestId, userId, requestData]);
        return { request_id: requestId, status: 'PENDING' };
    }
    async function fulfillDataAccessRequest(userId, requestId) {
        const requestRes = await server.db.query(`SELECT id, status FROM public.data_access_requests WHERE request_id = $1 AND user_id = $2`, [requestId, userId]);
        ...
        await server.db.query(`UPDATE public.data_access_requests SET status = 'COMPLETED', completed_at = NOW(), request_data = $2 WHERE request_id = $1`, [requestId, responseData]);
        return responseData;
    }
    async function submitDataErasureRequest(userId, note) {
        const requestId = generateComplianceRequestId();
        await server.db.query(`INSERT INTO public.data_erasure_requests (request_id, user_id, status, requested_at, note)
       VALUES ($1,$2,'PENDING',NOW(),$3)`, [requestId, userId, note]);
        return { request_id: requestId, status: 'PENDING' };
    }
    return {
        createConsent,
