...
export function createFarmerService(server) {
    ...
    async function sendWhatsAppWelcome(phone, kisanId) {
    ...
    return {
        async registerFarmer(userId, payload) {
            ...
            await server.db.query('UPDATE public.users SET language = $1, role = $2, updated_at = NOW() WHERE id = $3', [data.language, 'FARMER', userId]);
            await server.db.query(`INSERT INTO public.farmer_profiles (user_id, state_code, district, geo_lat, geo_lng, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())
         ON CONFLICT (user_id) DO UPDATE SET state_code = EXCLUDED.state_code, district = EXCLUDED.district, geo_lat = EXCLUDED.geo_lat, geo_lng = EXCLUDED.geo_lng`, [userId, data.state_code.toUpperCase(), data.district, data.geo_lat, data.geo_lng]);
            return { kisanId: user.kisan_id, status: 'PENDING_KYC', nextStep: 'kyc' };
        },
        async initiateDigiLockerKyc(userId) {
            const user = await findUser(userId);
            if (!user) {
        ...
        async completeDigiLockerCallback(code, userId) {
            ...
            await server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, aadhaar_encrypted, pan_encrypted, digilocker_ref, created_at)
         VALUES ($1, encrypt($2, $3, 'aes'), encrypt($4, $3, 'aes'), $5, NOW())
         ON CONFLICT (farmer_id) DO UPDATE SET aadhaar_encrypted = encrypt($2, $3, 'aes'), pan_encrypted = encrypt($4, $3, 'aes'), digilocker_ref = $5, kyc_completed_at = NOW()`, [userId, aadhaarData.aadhaarNumber, encryptionKey, panNumber ?? '', code]);
            const kisanId = await generateUniqueKisanId(server, profile.state_code);
            await server.db.query('UPDATE public.users SET kyc_status = $1, kisan_id = $2, updated_at = NOW() WHERE id = $3', ['ACTIVE', kisanId, userId]);
            const user = await findUser(userId);
            if (user) {
        ...
        async addBankAccount(userId, payload) {
            ...
            const fundJson = (await fundResponse.json());
            ...
            await server.db.query(`INSERT INTO vault.farmer_kyc (farmer_id, bank_account_token, bank_ifsc, bank_verified, penny_drop_ref, created_at)
         VALUES ($1, $2, $3, FALSE, $4, NOW())
         ON CONFLICT (farmer_id) DO UPDATE SET bank_account_token = $2, bank_ifsc = $3, bank_verified = FALSE, penny_drop_ref = $4`, [userId, fundAccountId, data.ifsc, pennyDropRef]);
            return { status: 'VERIFICATION_PENDING', message: 'Bank account verification in progress', kisanId: user.kisan_id };
        },
        async previewFpoBulkRegister(fpoRegistrationNumber, csvBuffer) {
        ...
        async confirmFpoBulkRegister(userId, jobId, confirmed) {
            ...
            const previewData = JSON.parse(previewDataRaw);
        ...
        async getBulkRegisterStatus(jobId) {
