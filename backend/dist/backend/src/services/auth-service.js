...
async function sendOtpViaMsg91(server, phone, otp) {
...
export function createAuthService(server) {
    async function createTokenPair(user) {
        ...
        await server.db.query(`INSERT INTO public.refresh_tokens (user_id, token_hash, expires_at, revoked, created_at) VALUES ($1, $2, NOW() + interval '30 days', FALSE, NOW())`, [user.id, tokenHash]);
    ...
    return {
        async requestOtp(phone) {
            ...
            await server.db.query(`INSERT INTO public.otp_sessions (phone, otp_hash, attempts, expires_at, used, created_at) VALUES ($1, $2, 0, NOW() + interval '10 minutes', FALSE, NOW())`, [validated.phone, otpHash]);
        ...
        async verifyOtp(phone, otp) {
            ...
            const result = await server.db.query('SELECT * FROM public.otp_sessions WHERE phone = $1 AND used = FALSE AND expires_at > NOW() ORDER BY created_at DESC LIMIT 1', [validated.phone]);
            ...
            const existingUser = await server.db.query('SELECT id, phone, role, kisan_id, kyc_status FROM public.users WHERE phone = $1', [validated.phone]);
            let user = existingUser.rows[0];
            let isNewUser = false;
            if (!user) {
                const created = await server.db.query('INSERT INTO public.users (phone, role, kyc_status, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW()) RETURNING id, phone, role, kisan_id, kyc_status', [validated.phone, 'FARMER', 'PENDING_KYC']);
                user = created.rows[0];
                isNewUser = true;
            }
        ...
        async refreshToken(refreshToken) {
            ...
            const result = await server.db.query('SELECT * FROM public.refresh_tokens WHERE token_hash = $1 AND revoked = FALSE AND expires_at > NOW() LIMIT 1', [tokenHash]);
            ...
            const userResult = await server.db.query('SELECT id, phone, role, kisan_id, kyc_status FROM public.users WHERE id = $1', [row.user_id]);
        ...
        async logout(refreshToken, userId) {
            const validated = refreshTokenSchema.parse({ refreshToken });
            const tokenHash = hashRefreshToken(validated.refreshToken);
            await server.db.query('UPDATE public.refresh_tokens SET revoked = TRUE WHERE token_hash = $1 AND user_id = $2', [tokenHash, userId]);
            return { success: true };
        }
    };
}
