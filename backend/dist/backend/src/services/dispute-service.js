...
export function createDisputeService(server) {
    async function writeLedgerEntry(entry) {
        ...
        await server.db.query(`INSERT INTO audit.transaction_ledger(txn_id, order_id, event_type, amount_paise, farmer_id, buyer_id, metadata, prev_hash, entry_hash, created_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW())`, [`TXN-${crypto.randomBytes(6).toString('hex')}`, entry.order_id, entry.event_type, entry.amount_paise, entry.farmer_id ?? null, entry.buyer_id ?? null, entry.metadata ?? {}, previousHash, entryHash]);
        return entryHash;
    }
    async function assignAgent(disputeId) {
        const agentResult = await server.db.query(`SELECT id FROM public.users WHERE role = 'OPERATIONS' ORDER BY last_active_at NULLS LAST, created_at ASC LIMIT 1`);
        const agent = agentResult.rows[0];
        if (!agent) {
            await server.db.query(`UPDATE public.disputes SET status = 'PENDING_ASSIGNMENT', updated_at = NOW() WHERE dispute_id = $1`, [disputeId]);
        ...
        await server.db.query(`UPDATE public.disputes SET assigned_agent_id = $1, agent_assigned_at = NOW(), status = 'AGENT_ASSIGNED', updated_at = NOW() WHERE dispute_id = $2`, [agent.id, disputeId]);
        await logDisputeAudit(disputeId, 'EVIDENCE_SUBMITTED', 'AGENT_ASSIGNED', agent.id, 'operations', 'Agent assigned automatically');
        return agent.id;
    }
    async function logDisputeAudit(disputeId, fromStatus, toStatus, actorId, actorRole, notes) {
        ...
        if (!dispute)
            return;
        await server.db.query(`INSERT INTO public.dispute_audit_log (dispute_id, from_status, to_status, actor_id, actor_role, notes)
       VALUES ($1, $2, $3, $4, $5, $6)`, [dispute.id, fromStatus, toStatus, actorId, actorRole, notes]);
    }
    async function createDispute(buyerId, orderId, reason, description) {
        const orderResult = await server.db.query(`SELECT o.id, o.order_id, o.farmer_id, o.buyer_id, o.delivery_confirmed_at, o.order_status, o.payment_status, o.subtotal_paise
       ...
       WHERE o.order_id = $1`, [orderId]);
        ...
        if (order.buyer_id !== buyerId)
            throw server.httpErrors.forbidden('Only the buyer can raise a dispute');
        if (!order.delivery_confirmed_at)
            throw server.httpErrors.badRequest('Delivery has not been confirmed yet');
        ...
        try {
            await server.db.query(`UPDATE public.orders SET payment_status = 'DISPUTE_FROZEN', updated_at = NOW() WHERE id = $1`, [order.id]);
            ...
            const insert = await server.db.query(`INSERT INTO public.disputes (dispute_id, order_id, raised_by, farmer_id, reason, description, status, evidence_deadline_at, created_at, updated_at)
         VALUES ($1,$2,$3,$4,$5,$6,'AWAITING_EVIDENCE',$7,NOW(),NOW())
         RETURNING dispute_id, status, reason, description, evidence_deadline_at`, [disputeId, order.id, buyerId, order.farmer_id, reason, description, evidenceDeadline]);
    ...
    async function getEvidenceUploadUrls(userId, disputeId, photoCount) {
        const disputeResult = await server.db.query(`SELECT d.id, d.status, d.raised_by, d.evidence_deadline_at
       FROM public.disputes d
       WHERE d.dispute_id = $1`, [disputeId]);
        const dispute = disputeResult.rows[0];
        if (!dispute)
            throw server.httpErrors.notFound('Dispute not found');
        if (dispute.raised_by !== userId)
            throw server.httpErrors.forbidden('Only the buyer can upload evidence');
    ...
    async function confirmEvidenceUpload(userId, disputeId, s3Keys) {
        const disputeResult = await server.db.query(`SELECT d.id, d.status, d.raised_by, d.order_id
       FROM public.disputes d
       WHERE d.dispute_id = $1`, [disputeId]);
        const dispute = disputeResult.rows[0];
        if (!dispute)
            throw server.httpErrors.notFound('Dispute not found');
        if (dispute.raised_by !== userId)
            throw server.httpErrors.forbidden('Only the buyer can confirm evidence upload');
        if (dispute.status !== 'AWAITING_EVIDENCE') {
        ...
        await server.db.query('BEGIN');
        try {
            await server.db.query(`UPDATE public.disputes SET status = 'EVIDENCE_SUBMITTED', evidence_urls = $1, evidence_uploaded_at = NOW(), updated_at = NOW() WHERE dispute_id = $2`, [urls, disputeId]);
            await logDisputeAudit(disputeId, 'AWAITING_EVIDENCE', 'EVIDENCE_SUBMITTED', userId, 'buyer', 'Evidence uploaded and confirmed');
            for (const url of urls) {
                await server.db.query(`INSERT INTO public.dispute_evidence (dispute_id, uploaded_by, evidence_url, evidence_metadata)
           VALUES ((SELECT id FROM public.disputes WHERE dispute_id = $1), $2, $3, $4)`, [disputeId, userId, url, { source: 'buyer_confirmed' }]);
            }
            await server.db.query('COMMIT');
        }
        catch (err) {
    ...
    async function resolveDispute(actorId, disputeId, outcome, notes) {
        const disputeResult = await server.db.query(`SELECT d.id, d.status, d.order_id, d.farmer_id, d.buyer_id, o.id AS order_pk, o.payment_status
       FROM public.disputes d
       ...
       WHERE d.dispute_id = $1`, [disputeId]);
        ...
        if (!actor)
            throw server.httpErrors.notFound('Actor not found');
        ...
        await server.db.query(`UPDATE public.disputes SET status = $1, resolved_at = NOW(), resolved_by = $2, resolution_outcome = $3, updated_at = NOW() WHERE dispute_id = $4`, [outcomeStatus, actorId, notes, disputeId]);
        if (outcome === 'BUYER_FAVOR') {
            await server.db.query(`UPDATE public.orders SET payment_status = 'REFUNDED', order_status = 'DISPUTE_RESOLVED_BUYER', updated_at = NOW() WHERE id = $1`, [dispute.order_pk]);
            await writeLedgerEntry({
            ...
            await server.db.query(`UPDATE public.orders SET order_status = 'DISPUTE_RESOLVED_FARMER', updated_at = NOW() WHERE id = $1`, [dispute.order_pk]);
            await writeLedgerEntry({
    ...
    async function autoCloseDisputeIfEvidenceMissing(disputeId) {
        ...
        if (dispute.status !== 'AWAITING_EVIDENCE')
            return null;
        await server.db.query(`UPDATE public.disputes SET status = 'AUTO_CLOSED_FARMER', resolved_at = NOW(), resolution_outcome = 'FARMER_FAVOR', updated_at = NOW() WHERE dispute_id = $1`, [disputeId]);
        await server.db.query(`UPDATE public.orders SET order_status = 'DISPUTE_RESOLVED_FARMER', updated_at = NOW() WHERE id = $1`, [dispute.order_id]);
        await writeLedgerEntry({
    ...
    async function autoResolveDisputeAfterDeadline(disputeId) {
        const disputeResult = await server.db.query(`SELECT d.id, d.status, d.order_id, d.farmer_id, d.buyer_id
       FROM public.disputes d
       WHERE d.dispute_id = $1`, [disputeId]);
        const dispute = disputeResult.rows[0];
        if (!dispute)
    ...
    async function enforceAgentAssignment(disputeId) {
    ...
    async function getDisputeById(userId, disputeId) {
        const result = await server.db.query(`SELECT d.*, o.order_id AS external_order_id, o.payment_status, o.order_status
       FROM public.disputes d
       JOIN public.orders o ON o.id = d.order_id
       WHERE d.dispute_id = $1 AND (d.buyer_id = $2 OR d.farmer_id = $2 OR d.assigned_agent_id = $2)`, [disputeId, userId]);
        const dispute = result.rows[0];
        if (!dispute)
            throw server.httpErrors.notFound('Dispute not found');
        return dispute;
    }
    async function listDisputesForUser(userId) {
        const result = await server.db.query(`SELECT d.dispute_id, d.status, d.reason, d.resolution_outcome, d.created_at, d.resolved_at, o.order_id AS external_order_id
       FROM public.disputes d
       JOIN public.orders o ON o.id = d.order_id
