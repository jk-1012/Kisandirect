...
export function createMarketService(server) {
    ...
    async function ingestAgmarknetPrices() {
        ...
        try {
            for (const row of rows) {
                await server.db.query(`INSERT INTO public.mandi_prices (crop_type, mandi_name, district, state_code, price_inr_per_kg, price_date, source, metadata)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
           ON CONFLICT (crop_type, state_code, mandi_name, price_date, source)
           DO UPDATE SET price_inr_per_kg = EXCLUDED.price_inr_per_kg, metadata = EXCLUDED.metadata, created_at = NOW()`, [row.crop_type, row.mandi_name, row.district || null, row.state_code, row.price_inr_per_kg, row.price_date, row.source, row.metadata ?? null]);
            }
        }
        catch (error) {
        ...
        const summary = await server.db.query(`WITH latest AS (
    ...
    async function triggerPriceAlerts(cropType, stateCode, currentPricePaise) {
        const alertsResult = await server.db.query(`SELECT a.id, a.farmer_id, a.direction, a.threshold_price_per_kg_inr, a.last_triggered_at, u.phone, u.language
       ...
       WHERE a.active = true AND a.crop_type = $1 AND a.state_code = $2
         AND ((a.direction = 'ABOVE' AND a.threshold_price_per_kg_inr * 100 <= $3)
              OR (a.direction = 'BELOW' AND a.threshold_price_per_kg_inr * 100 >= $3))`, [cropType, stateCode, currentPricePaise]);
        const now = new Date();
        let sent = 0;
    ...
    async function createPriceAlert(userId, payload) {
        const validated = priceAlertSchema.parse(payload);
        const insert = await server.db.query(`INSERT INTO public.price_alerts (farmer_id, crop_type, state_code, direction, threshold_price_per_kg_inr, active, created_at, updated_at)
       VALUES ($1,$2,$3,$4,$5,true,NOW(),NOW()) RETURNING id`, [userId, validated.crop_type, validated.state_code, validated.direction, validated.threshold_price_per_kg_inr]);
        return { alert_id: insert.rows[0].id };
    }
    async function listPriceAlerts(userId) {
        const result = await server.db.query(`SELECT id, crop_type, state_code, direction, threshold_price_per_kg_inr, active, last_triggered_at, created_at, updated_at
       FROM public.price_alerts WHERE farmer_id = $1 ORDER BY created_at DESC`, [userId]);
    ...
    async function getMarketIntelligence(cropType, stateCode) {
        const crop = normalizeCropType(cropType);
        const state = normalizeStateCode(stateCode);
        const latestResult = await server.db.query(`SELECT price_inr_per_kg, price_date, mandi_name, district FROM public.mandi_prices
       ...
       LIMIT 1`, [crop, state]);
        ...
        const weeklyResult = await server.db.query(`SELECT AVG(price_inr_per_kg) AS avg_price
       FROM public.mandi_prices
       WHERE crop_type = $1 AND state_code = $2 AND price_date >= CURRENT_DATE - INTERVAL '7 days'`, [crop, state]);
        const monthResult = await server.db.query(`SELECT AVG(price_inr_per_kg) AS avg_price
       FROM public.mandi_prices
       WHERE crop_type = $1 AND state_code = $2 AND price_date >= CURRENT_DATE - INTERVAL '30 days'`, [crop, state]);
        const trendResult = await server.db.query(`SELECT price_date, AVG(price_inr_per_kg) AS avg_price
       FROM public.mandi_prices
       WHERE crop_type = $1 AND state_code = $2 AND price_date >= CURRENT_DATE - INTERVAL '7 days'
       GROUP BY price_date
       ORDER BY price_date ASC`, [crop, state]);
        const trend = trendResult.rows.map((row) => ({ date: row.price_date?.toISOString().slice(0, 10) ?? null, average_price_per_kg_inr: Number(row.avg_price) }));
        ...
        return {
            ...
            trend,
            warning: weeklyAvg > 0 && Number(latest.price_inr_per_kg) > weeklyAvg * 1.15
