...
function formatCurrencyInr(paise) {
...
export function createTDSService(server) {
    async function calculateTDS(farmerId, payoutAmountPaise) {
    ...
    async function getFarmerWithKYC(farmerId) {
        const baseRes = await server.db.query(`SELECT u.id AS user_id, u.phone, u.kisan_id, u.kyc_status, fp.annual_payout_inr, fp.tds_deducted_inr,
       ...
       WHERE u.id = $1`, [farmerId]);
        ...
        if (encryptionKey && row.pan_encrypted) {
            const panRes = await server.db.query(`SELECT convert_from(decrypt(pan_encrypted, $1, 'aes'), 'UTF8') AS pan
         FROM vault.farmer_kyc WHERE farmer_id = $2`, [encryptionKey, farmerId]);
    ...
    async function getTDSDataForFY(farmerId, financialYear) {
        const { startDate, endDate } = parseFinancialYear(financialYear);
        const res = await server.db.query(`SELECT o.order_id, o.created_at AS order_date,
       ...
       WHERE o.farmer_id = $1
         AND t.created_at >= $2
         AND t.created_at < $3
       ORDER BY t.created_at ASC`, [farmerId, startDate, endDate]);
        ...
        const totalGrossPaise = records.reduce((sum, item) => sum + item.gross_paise, 0);
        const totalTdsPaise = records.reduce((sum, item) => sum + item.tds_paise, 0);
        const totalNetPaise = records.reduce((sum, item) => sum + item.net_paise, 0);
        const tdsByMonth = records.reduce((acc, item) => {
    ...
    function buildForm16AHtml(farmer, tdsData, financialYear) {
        const summaryRows = tdsData.records
            .map((record) => `<tr><td>${record.order_id}</td><td>${new Date(record.order_date).toISOString().slice(0, 10)}</td><td>${formatCurrencyInr(record.gross_paise)}</td><td>${formatCurrencyInr(record.tds_paise)}</td><td>${formatCurrencyInr(record.net_paise)}</td></tr>`)
            .join('');
        return `<!doctype html>
      <html>
      <head>
    ...
    async function generateForm16A(farmerId, financialYear) {
    ...
    async function getAdminSummary(financialYear) {
        const { startDate, endDate } = parseFinancialYear(financialYear);
        const tdsByMonthRes = await server.db.query(`SELECT to_char(t.created_at, 'YYYY-MM') AS month,
       ...
       WHERE t.event_type = 'ESCROW_RELEASED'
         ...
         AND t.created_at < $2
       GROUP BY 1
       ORDER BY 1`, [startDate, endDate]);
        const approachingRes = await server.db.query(`SELECT u.id, u.kisan_id, u.phone, fp.annual_payout_inr, fp.tds_deducted_inr,
       ...
       ORDER BY fp.annual_payout_inr DESC`, [Math.round(TDS_THRESHOLD_PAISE / 100 * ALERT_THRESHOLD_RATIO), Math.round(TDS_THRESHOLD_PAISE / 100)]);
        const summary = {
            financial_year: financialYear ?? `${startDate.slice(0, 4)}-${String(Number(startDate.slice(0, 4)) + 1).slice(-2)}`,
            tds_by_month: tdsByMonthRes.rows.map((row) => ({ month: row.month, tds_paise: Number(row.tds_paise) })),
            approaching_threshold: approachingRes.rows.map((row) => ({
                farmer_id: row.id,
                kisan_id: row.kisan_id,
                phone: row.phone,
                annual_payout_inr: Number(row.annual_payout_inr ?? 0),
    ...
    async function sendSlackAlert(farmers, financialYear) {
