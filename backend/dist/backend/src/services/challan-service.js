...
export function createChallanService(server) {
    async function createEChallan(orderId, actorId) {
        const res = await server.db.query(`SELECT o.*, l.crop_type, l.crop_type AS crop_type_display, l.harvest_date, l.grade, u.phone AS buyer_phone, u.language AS buyer_language,
       ...
       WHERE o.order_id = $1`, [orderId]);
        ...
        try {
            ...
            if (existing.rowCount > 0) {
                await server.db.query(`UPDATE public.e_challans
           ...
           WHERE order_id = $4`, [verificationToken, challanUrl, 'GENERATED', order.id]);
            }
            else {
                await server.db.query(`INSERT INTO public.e_challans
           (order_id, verification_token, challan_url, status, generated_at, created_at, updated_at)
           VALUES ($1,$2,$3,$4,NOW(),NOW(),NOW())`, [order.id, verificationToken, challanUrl, 'GENERATED']);
            }
    ...
    async function signEChallan(orderId, actorId) {
        const res = await server.db.query(`SELECT ec.*, o.order_id, o.farmer_id, o.buyer_id, u.phone AS buyer_phone
       ...
       WHERE o.order_id = $1`, [orderId]);
        ...
        if (!challan)
            throw server.httpErrors.notFound('e-Challan not found for order');
        if (actorId !== challan.farmer_id)
            throw server.httpErrors.forbidden('Only farmer may initiate challan signing');
       ...
       WHERE id = $4`, ['AWAITING_BUYER_OTP', otpHash, expiresAt, challan.id]);
    ...
    async function verifyEChallanOtp(orderId, actorId, otp) {
        const res = await server.db.query(`SELECT ec.*, o.order_id, o.buyer_id, o.farmer_id
       FROM public.e_challans ec
       JOIN public.orders o ON o.id = ec.order_id
       WHERE o.order_id = $1`, [orderId]);
        const challan = res.rows[0];
        if (!challan)
            throw server.httpErrors.notFound('e-Challan not found for order');
        if (actorId !== challan.buyer_id)
            throw server.httpErrors.forbidden('Only buyer may verify the challan OTP');
        ...
        try {
         ...
         WHERE id = $2`, ['COMPLETED', challan.id]);
    ...
    async function getEChallanByToken(token) {
        const res = await server.db.query(`SELECT ec.status, ec.challan_url, ec.generated_at, ec.farmer_signed_at, ec.buyer_verified_at,
       ...
       JOIN public.users u ON u.id = o.buyer_id
       LEFT JOIN public.farmer_profiles up ON up.user_id = u.id
       WHERE ec.verification_token = $1`, [token]);
        const row = res.rows[0];
        if (!row)
            throw server.httpErrors.notFound('e-Challan not found');
        return {
    ...
    function buildChallanHtml(order, verificationUrl, status) {
        const formatDate = (date) => date.toLocaleDateString('en-IN');
        const formatDateTime = (date) => date.toLocaleString('en-IN');
        return `<!DOCTYPE html>
<html>
