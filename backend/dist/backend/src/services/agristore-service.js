...
export function createAgriStoreService(server) {
    const pageCollection = server.mongo.db.collection('agristore_pages');
    const analyticsCollection = server.mongo.db.collection('agristore_analytics');
    async function normalizeSlug(slug) {
...
<body>
  <div class="container">
    ...
    async function uploadHtml(slug, html) {
        const s3Client = server.storage.s3Client;
    ...
    async function recordPageView(slug, userAgent, ip) {
        const normalized = await normalizeSlug(slug);
        const date = new Date().toISOString().slice(0, 10);
        await analyticsCollection.updateOne({ slug: normalized, date }, {
            $inc: { views: 1 },
    ...
    async function generateQrCodeUrl(slug) {
