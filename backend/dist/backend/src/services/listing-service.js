...
const validCropTypes = new Set(Object.values(CROP_TAXONOMY).flatMap((category) => Object.keys(category)));
...
const cropLabelMap = new Map(Object.entries(VISION_LABEL_MAP).map(([label, cropType]) => [label.toLowerCase(), cropType]));
...
function getCropDisplayName(cropCategory, cropType) {
    ...
    const crop = category[cropType];
    return crop?.label?.en ?? cropType;
}
function findCropTypeFromLabel(label) {
...
export function createListingService(server) {
    ...
    async function getListingById(listingId) {
        const result = await server.db.query(`SELECT l.*, u.kisan_id, u.trust_score, fp.state_code, fp.district
       FROM public.listings l
       LEFT JOIN public.users u ON u.id = l.farmer_id
       LEFT JOIN public.farmer_profiles fp ON fp.user_id = l.farmer_id
       WHERE l.listing_id = $1`, [listingId]);
    ...
    async function cancelListing(userId, listingId) {
        const result = await server.db.query(`SELECT id, job_ids FROM public.listings WHERE listing_id = $1 AND farmer_id = $2 AND status = 'ACTIVE'`, [listingId, userId]);
        if (result.rows.length === 0) {
            throw server.httpErrors.notFound('Active listing not found for cancellation');
        }
        const listing = result.rows[0];
        if (listing.job_ids) {
            await removeListingLifecycleJobs(server, listing.job_ids);
        }
        await server.db.query(`UPDATE public.listings SET status = 'CANCELLED', updated_at = NOW() WHERE id = $1`, [listing.id]);
        try {
    ...
    async function getPhotoUploadUrl(userId, filename, contentType) {
    ...
    async function processPhoto(userId, s3Key) {
        const bucketName = getListingBucketName();
        const job = await server.queues.photoProcessingQueue.add('PROCESS_LISTING_PHOTO', { userId, s3Key, bucketName }, { removeOnComplete: true, removeOnFail: false });
        try {
    ...
    async function searchListings(query) {
        ...
        if (query.price_min !== undefined || query.price_max !== undefined) {
            ...
            if (query.price_min !== undefined)
                range.gte = Math.round(query.price_min * 100);
            if (query.price_max !== undefined)
                range.lte = Math.round(query.price_max * 100);
        ...
        if (query.harvest_date_from || query.harvest_date_to) {
            const range = {};
            if (query.harvest_date_from)
                range.gte = query.harvest_date_from;
            if (query.harvest_date_to)
                range.lte = query.harvest_date_to;
        ...
        const results = hits.map((hit) => {
            ...
            return {
                ...
                distance_km: query.sort === 'proximity' && hit.sort && typeof hit.sort[0] === 'number'
                    ? Number(hit.sort[0])
                    : null,
    ...
    async function getFarmerListings(userId, status, page = 1, limit = 20) {
        ...
        const rows = await server.db.query(`SELECT l.*, COALESCE(o.order_count,0) as order_count, COALESCE(o.revenue_paise,0) as revenue_paise,
    ...
    async function updateListing(userId, listingId, payload) {
        ...
        const listing = result.rows[0];
        if (listing.farmer_id !== userId)
            throw server.httpErrors.forbidden('Not owner of listing');
        if (listing.status !== 'ACTIVE')
            throw server.httpErrors.badRequest('Only ACTIVE listings can be updated');
        if (listing.job_ids) {
            await removeListingLifecycleJobs(server, listing.job_ids);
        }
        const fields = [];
    ...
    async function deleteListing(userId, listingId) {
        ...
        const orders = await server.db.query(`SELECT COUNT(*) as cnt FROM public.orders WHERE listing_id = $1 AND (payment_status = 'PENDING' OR order_status = 'PENDING' OR payment_status = 'ESCROW_HELD' OR order_status = 'ESCROW_HELD')`, [listing.id]);
