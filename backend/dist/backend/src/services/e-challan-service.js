...
import { EChallanStatus, ChallanSignatureType, ChallanAuditAction, EChallanError, EChallanErrorCodes, VerificationMethod, VALID_STATUS_TRANSITIONS, } from '../types/e-challan';
import { getChallanTemplate } from './e-challan-template';
export class EChallanService {
    constructor(server, db, redis, s3Client, config) {
        this.browser = null;
        this.server = server;
        this.db = db;
        this.redis = redis;
    ...
    async generateQrCode(data) {
        try {
            ...
            throw new EChallanError(EChallanErrorCodes.PDF_GENERATION_FAILED, 'Failed to generate QR code', 500);
        }
    }
    /**
     * Create challan document
     */
    async createChallan(request) {
        const client = await this.db.connect();
        try {
            // Fetch order details
            const orderResult = await client.query(`SELECT o.id, o.farmer_id, o.buyer_id, o.created_at,
         ...
         WHERE o.id = $1`, [request.orderId]);
            if (orderResult.rows.length === 0) {
                throw new EChallanError(EChallanErrorCodes.CHALLAN_NOT_FOUND, 'Order not found', 404);
            }
            const order = orderResult.rows[0];
            // Generate challan number
            const challanNumberResult = await client.query(`SELECT vault.generate_challan_number() as challan_number`);
            ...
            // Fetch order items
            const itemsResult = await client.query(`SELECT name, quantity, unit, price_per_unit, total_price
         FROM public.order_items
         WHERE order_id = $1`, [request.orderId]);
            ...
            const insertResult = await client.query(`INSERT INTO vault.e_challans (
        ...
        RETURNING id, status, created_at`, [
            ...
            // Create audit log
            await this.createAuditLog(challanId, ChallanAuditAction.CREATED, undefined, EChallanStatus.DRAFT, undefined, 'SYSTEM', { challanNumber });
    ...
    async sendOtp(request) {
        ...
        try {
            // Fetch challan
            const challanResult = await client.query(`SELECT id, status, farmer_id, buyer_id FROM vault.e_challans WHERE id = $1`, [request.challanId]);
            if (challanResult.rows.length === 0) {
                throw new EChallanError(EChallanErrorCodes.CHALLAN_NOT_FOUND, 'Challan not found', 404);
            }
            const challan = challanResult.rows[0];
            // Verify status transition
            if (!VALID_STATUS_TRANSITIONS[challan.status].includes(EChallanStatus.OTP_SENT)) {
                throw new EChallanError(EChallanErrorCodes.INVALID_STATUS_TRANSITION, `Cannot send OTP from status ${challan.status}`, 400);
            }
            // Generate OTP
            ...
            await this.redis.setex(redisKey, this.config.otpExpirySeconds, JSON.stringify({
                otpCode,
            ...
            await client.query(`INSERT INTO vault.otp_cache (challan_id, otp_code, recipient_phone, recipient_email, expires_at, max_attempts)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (challan_id) DO UPDATE SET
         otp_code = $2, expires_at = $5, verification_attempts = 0`, [request.challanId, otpCode, request.phoneNumber, request.email, expiresAt, this.config.otpMaxAttempts]);
            // Update challan status
            const now = new Date();
            await client.query(`UPDATE vault.e_challans SET status = $1, otp_code = $2, otp_sent_at = $3, otp_attempts = 0 WHERE id = $4`, [EChallanStatus.OTP_SENT, otpCode, now, request.challanId]);
            // Create audit log
            await this.createAuditLog(request.challanId, ChallanAuditAction.OTP_SENT, challan.status, EChallanStatus.OTP_SENT, undefined, 'SYSTEM', { recipientType: request.recipientType, phoneNumber: request.phoneNumber });
    ...
    async verifyOtp(request) {
        ...
        try {
            // Fetch challan
            const challanResult = await client.query(`SELECT id, status, otp_attempts, otp_max_attempts FROM vault.e_challans WHERE id = $1`, [request.challanId]);
            if (challanResult.rows.length === 0) {
                throw new EChallanError(EChallanErrorCodes.CHALLAN_NOT_FOUND, 'Challan not found', 404);
            }
            const challan = challanResult.rows[0];
            ...
            if (challan.otp_attempts >= challan.otp_max_attempts) {
                throw new EChallanError(EChallanErrorCodes.OTP_MAX_ATTEMPTS_EXCEEDED, 'Maximum OTP attempts exceeded', 429);
            }
            // Fetch OTP from Redis
            const redisKey = `${this.config.redisKeyPrefix}:otp:${request.challanId}`;
            const cachedOtp = await this.redis.get(redisKey);
            if (!cachedOtp) {
                throw new EChallanError(EChallanErrorCodes.OTP_EXPIRED, 'OTP has expired', 400);
            }
            ...
            if (otpData.otpCode !== request.otpCode) {
                ...
                await client.query(`UPDATE vault.e_challans SET otp_attempts = $1 WHERE id = $2`, [newAttempts, request.challanId]);
                throw new EChallanError(EChallanErrorCodes.OTP_INVALID, `Invalid OTP. ${challan.otp_max_attempts - newAttempts} attempts remaining`, 400);
            }
            // OTP verified - update challan status
            const now = new Date();
            await client.query(`UPDATE vault.e_challans SET status = $1, otp_verified_at = $2, otp_attempts = 0 WHERE id = $3`, [EChallanStatus.OTP_VERIFIED, now, request.challanId]);
            // Remove OTP from Redis
            await this.redis.del(redisKey);
            // Create audit log
            await this.createAuditLog(request.challanId, ChallanAuditAction.OTP_VERIFIED, challan.status, EChallanStatus.OTP_VERIFIED, undefined, 'USER', { recipientType: request.recipientType });
    ...
    async signChallan(request) {
        ...
        try {
            ...
            if (challan.status !== EChallanStatus.OTP_VERIFIED) {
                throw new EChallanError(EChallanErrorCodes.INVALID_STATUS_TRANSITION, `Challan must be OTP verified before signing. Current status: ${challan.status}`, 400);
            }
            // Verify signer
            ...
            if (request.signerType === ChallanSignatureType.BUYER && request.signerType !== ChallanSignatureType.BUYER) {
                throw new EChallanError(EChallanErrorCodes.UNAUTHORIZED, 'Unauthorized signer', 403);
            }
            ...
            if (signatureBuffer.length > this.config.signatureMaxSizeKB * 1024) {
                throw new EChallanError(EChallanErrorCodes.INVALID_REQUEST, `Signature exceeds maximum size of ${this.config.signatureMaxSizeKB}KB`, 400);
            }
            // Calculate signature hash
            const signatureHash = this.calculateHash(request.signatureImageBase64);
            // Insert signature
            const signatureResult = await client.query(`INSERT INTO vault.e_challan_signatures (
        ...
        RETURNING id, signature_timestamp`, [
            ...
            if (request.signerType === ChallanSignatureType.BUYER) {
                ...
                const buyerSignatureResult = await client.query(`SELECT id FROM vault.e_challan_signatures
           WHERE challan_id = $1 AND signer_type = $2`, [request.challanId, ChallanSignatureType.BUYER]);
            ...
            // Update challan status
            await client.query(`UPDATE vault.e_challans SET status = $1, modified_by = $2, completed_at = $3 WHERE id = $4`, [newStatus, expectedSignerId, isComplete ? new Date() : null, request.challanId]);
            // Create audit log
            await this.createAuditLog(request.challanId, ChallanAuditAction.SIGNATURE_CAPTURED, challan.status, newStatus, expectedSignerId, 'USER', { signerType: request.signerType, signatureHash });
     ...
     */
    async completeChallan(challanId) {
        const client = await this.db.connect();
        try {
            // Fetch challan with content
            const challanResult = await client.query(`SELECT id, challan_html, order_id FROM vault.e_challans WHERE id = $1`, [challanId]);
            if (challanResult.rows.length === 0) {
            ...
            await client.query(`UPDATE vault.e_challans SET challan_pdf_path = $1, final_hash = $2, status = $3, completed_at = $4
         WHERE id = $5`, [s3Path, finalHash, EChallanStatus.COMPLETED, new Date(), challanId]);
            // Trigger escrow release
            await this.triggerEscrowRelease(challan.order_id);
            // Create completion audit log
            await this.createAuditLog(challanId, ChallanAuditAction.COMPLETED, EChallanStatus.FARMER_SIGNED, EChallanStatus.COMPLETED, undefined, 'SYSTEM', { pdfPath: s3Path, finalHash });
     ...
     * Generate signed PDF
     */
    async generateSignedPdf(htmlContent) {
        try {
            ...
            throw new EChallanError(EChallanErrorCodes.PDF_GENERATION_FAILED, 'Failed to generate PDF', 500);
        }
    }
    /**
     * Upload to S3
     */
    async uploadToS3(challanId, pdfBuffer) {
        try {
            ...
            await this.db.query(`INSERT INTO vault.challan_s3_archive (
          ...
          server_side_encryption, is_public, access_control_list, https_url, file_hash
            ...
            throw new EChallanError(EChallanErrorCodes.S3_UPLOAD_FAILED, 'Failed to upload PDF to S3', 500);
        }
    }
    /**
     * Download challan
     */
    async downloadChallan(request) {
        const client = await this.db.connect();
        try {
            // Fetch challan
            const challanResult = await client.query(`SELECT id, challan_number, status, challan_pdf_path, final_hash, completed_at
         FROM vault.e_challans WHERE id = $1`, [request.challanId]);
            if (challanResult.rows.length === 0) {
                throw new EChallanError(EChallanErrorCodes.CHALLAN_NOT_FOUND, 'Challan not found', 404);
            }
            const challan = challanResult.rows[0];
            if (!challan.challan_pdf_path) {
                throw new EChallanError(EChallanErrorCodes.INVALID_REQUEST, 'PDF not yet generated', 400);
            }
            // Fetch signatures
            const signaturesResult = await client.query(`SELECT id, challan_id, signer_type, signer_name, signature_timestamp, verified_at
         FROM vault.e_challan_signatures WHERE challan_id = $1`, [request.challanId]);
            // Fetch audit log if requested
            let auditLog = [];
            if (request.includeHistory) {
                const auditResult = await client.query(`SELECT id, challan_id, action, previous_status, new_status, actor_id, actor_type,
                  change_details, ip_address, created_at
           FROM vault.challan_audit_log WHERE challan_id = $1 ORDER BY created_at`, [request.challanId]);
     ...
     */
    async createAuditLog(challanId, action, previousStatus, newStatus, actorId, actorType, changeDetails) {
        const client = await this.db.connect();
        try {
            const actionHash = this.calculateHash(`${challanId}${action}${previousStatus || ''}${newStatus || ''}${actorType}${Date.now()}`);
            await client.query(`INSERT INTO vault.challan_audit_log (
          challan_id, action, previous_status, new_status, actor_id, actor_type,
          change_details, action_hash
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [challanId, action, previousStatus, newStatus, actorId, actorType, JSON.stringify(changeDetails), actionHash]);
        }
        catch (error) {
     ...
     * Trigger escrow release
     */
    async triggerEscrowRelease(orderId) {
        try {
     ...
     */
    async checkIntegrity(challanId) {
        const client = await this.db.connect();
        try {
            const challanResult = await client.query(`SELECT content_hash, final_hash, challan_html FROM vault.e_challans WHERE id = $1`, [challanId]);
            if (challanResult.rows.length === 0) {
                throw new EChallanError(EChallanErrorCodes.CHALLAN_NOT_FOUND, 'Challan not found', 404);
            }
            const challan = challanResult.rows[0];
            ...
            const signaturesResult = await client.query(`SELECT signature_hash, signature_image_base64 FROM vault.e_challan_signatures WHERE challan_id = $1`, [challanId]);
            const allSignaturesValid = signaturesResult.rows.every((sig) => sig.signature_hash === this.calculateHash(sig.signature_image_base64));
            return {
                isValid: contentHashMatch && allSignaturesValid,
                challanStatus: (await client.query(`SELECT status FROM vault.e_challans WHERE id = $1`, [challanId])).rows[0].status,
     ...
     * Archive challan
     */
    async archiveChallan(challanId) {
        const client = await this.db.connect();
        try {
            const challanResult = await client.query(`SELECT status FROM vault.e_challans WHERE id = $1`, [challanId]);
            if (challanResult.rows.length === 0) {
                throw new EChallanError(EChallanErrorCodes.CHALLAN_NOT_FOUND, 'Challan not found', 404);
            }
            const challan = challanResult.rows[0];
            if (challan.status !== EChallanStatus.COMPLETED) {
                throw new EChallanError(EChallanErrorCodes.INVALID_STATUS_TRANSITION, 'Only completed challans can be archived', 400);
            }
            const now = new Date();
            await client.query(`UPDATE vault.e_challans SET status = $1, archived_at = $2 WHERE id = $3`, [EChallanStatus.ARCHIVED, now, challanId]);
            await this.createAuditLog(challanId, ChallanAuditAction.ARCHIVED, EChallanStatus.COMPLETED, EChallanStatus.ARCHIVED, undefined, 'SYSTEM', { archivedAt: now });
        }
        catch (error) {
 ...
 */
export function createEChallanService(server, db, redis, s3Client, config) {
