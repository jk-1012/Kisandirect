...
export default async function (server) {
    ...
    server.get('/transactions/:orderId/ledger', { preHandler: [server.authenticate] }, async (request, reply) => {
        ...
        if (String(order.farmer_id) !== String(userId) && request.user.role !== 'ADMIN')
        ...
        const entries = rows.rows.map((r) => ({ event_type: r.event_type, amount_inr: (Number(r.amount_paise) / 100).toFixed(2), timestamp: r.created_at, entry_hash: r.entry_hash, prev_hash: r.prev_hash }));
    ...
    server.get('/farmers/me/transactions', { preHandler: [server.authenticate] }, async (request, reply) => {
        const userId = request.user.userId;
        const q = querySchema.parse(request.query);
        const page = q.page ?? 1;
        const limit = q.limit ?? 20;
        const offset = (page - 1) * limit;
        const rows = await server.db.query(`SELECT o.order_id, o.created_at, l.crop_type, fp.district AS buyer_district, o.quantity_kg, o.subtotal_paise, o.platform_fee_paise, o.order_status, o.payment_status
       FROM public.orders o
       LEFT JOIN public.listings l ON l.id = o.listing_id
       LEFT JOIN public.farmer_profiles fp ON fp.user_id = o.buyer_id
       WHERE o.farmer_id = $1 ORDER BY o.created_at DESC LIMIT $2 OFFSET $3`, [userId, limit, offset]);
        const transactions = rows.rows.map((r) => ({ order_id: r.order_id, date: r.created_at, crop: r.crop_type, buyer_district: r.buyer_district, quantity_kg: Number(r.quantity_kg), gross_amount_inr: (Number(r.subtotal_paise) / 100).toFixed(2), platform_fee_inr: (Number(r.platform_fee_paise) / 100).toFixed(2), status: r.order_status }));
    ...
    server.get('/farmers/me/transactions/pdf', { preHandler: [server.authenticate] }, async (request, reply) => {
        const userId = request.user.userId;
        ...
        const lines = rows.rows.map((r) => `<tr><td>${r.order_id}</td><td>${new Date(r.created_at).toISOString().slice(0, 10)}</td><td>₹${(Number(r.subtotal_paise) / 100).toFixed(2)}</td></tr>`).join('');
