...
export default async function kycRoutes(server) {
     ...
     */
    server.post('/farmers/kyc/initiate', { preHandler: server.authenticate }, async (request, reply) => {
        try {
            ...
            const farmerResult = await server.db.query('SELECT id FROM farmers WHERE id = $1 AND user_id = $2', [payload.farmerId, userId]);
            ...
            await server.queues.connection.setEx(`kyc:session:metadata:${sessionId}`, (authRequest.expiresIn || 600) * 2, JSON.stringify(sessionMetadata));
     ...
     */
    server.get('/farmers/kyc/callback', async (request, reply) => {
        try {
            ...
            await server.db.query(`UPDATE farmers SET kyc_status = $1, kyc_verified_at = NOW() WHERE id = $2`, ['VERIFIED', kycSession.farmerId]);
            // Update user KYC status
            await server.db.query(`UPDATE users SET kyc_status = $1, updated_at = NOW() WHERE id = $2`, ['VERIFIED', kycSession.userId]);
            // Store KYC session in Redis for subsequent status checks
            await server.queues.connection.setEx(`kyc:completed:${kycSession.sessionId}`, 86400, // 24 hours
            JSON.stringify(kycSession));
            server.log.info({ farmerId: kycSession.farmerId, documentsCount: kycSession.documentsFetched?.length || 0 }, 'KYC completed successfully');
     ...
     */
    server.get('/farmers/kyc/status/:sessionId', async (request, reply) => {
        try {
     ...
     */
    server.get('/farmers/kyc/me', { preHandler: server.authenticate }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const kycResult = await server.db.query(`SELECT kyc_status, kyc_verified_at, kyc_expires_at FROM farmers WHERE user_id = $1 LIMIT 1`, [userId]);
     ...
     */
    server.post('/farmers/kyc/renew', { preHandler: server.authenticate }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            // Get farmer's ID
            const farmerResult = await server.db.query(`SELECT id FROM farmers WHERE user_id = $1`, [userId]);
            if (farmerResult.rows.length === 0) {
                return reply.code(404).send({
                    error: 'FARMER_NOT_FOUND'
                });
     ...
     */
    server.get('/farmers/kyc/document/:documentType', { preHandler: server.authenticate }, async (request, reply) => {
        try {
            ...
            const result = await server.db.query(`SELECT ${fieldName} FROM vault.farmer_kyc WHERE farmer_id = $1`, [farmerId]);
