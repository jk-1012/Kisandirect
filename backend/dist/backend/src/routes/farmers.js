...
export default async function (server) {
    ...
    server.post('/register', { preHandler: server.authenticate }, async (request, reply) => {
        const payload = registerSchema.parse(request.body);
        const userId = request.user.userId;
        const response = await farmerService.registerFarmer(userId, payload);
        return reply.code(200).send(response);
    });
    server.get('/kyc/initiate', { preHandler: server.authenticate }, async (request, reply) => {
        const userId = request.user.userId;
        const response = await farmerService.initiateDigiLockerKyc(userId);
        return reply.send(response);
    });
    server.get('/kyc/callback', async (request, reply) => {
    ...
    server.post('/bank/add', { preHandler: [server.authenticate, server.requireKYC] }, async (request, reply) => {
        const payload = bankSchema.parse(request.body);
        const userId = request.user.userId;
        const response = await farmerService.addBankAccount(userId, payload);
        return reply.send(response);
    });
    server.get('/me/listings', { preHandler: server.authenticate }, async (request, reply) => {
    ...
    server.post('/fpo/bulk-register', { preHandler: server.authenticate }, async (request, reply) => {
    ...
    server.post('/fpo/bulk-register/confirm', { preHandler: server.authenticate }, async (request, reply) => {
        if (request.user.role !== 'FPO_ADMIN') {
            return reply.code(403).send({ error: 'FORBIDDEN' });
        }
    ...
    server.get('/fpo/bulk-register/status/:job_id', { preHandler: server.authenticate }, async (request, reply) => {
        if (request.user.role !== 'FPO_ADMIN') {
            return reply.code(403).send({ error: 'FORBIDDEN' });
        }
