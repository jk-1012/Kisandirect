...
export default async function (server) {
    ...
    server.post('/disputes', { preHandler: [server.authenticate] }, async (request, reply) => {
    ...
    server.post('/disputes/:disputeId/evidence', { preHandler: [server.authenticate] }, async (request, reply) => {
        const { disputeId } = request.params;
        const userId = request.user.userId;
        const payload = evidenceRequestSchema.parse(request.body);
        const evidence = await disputeService.getEvidenceUploadUrls(userId, disputeId, payload.photo_count);
        return reply.code(201).send(evidence);
    });
    server.post('/disputes/:disputeId/evidence/confirm', { preHandler: [server.authenticate] }, async (request, reply) => {
        const { disputeId } = request.params;
        const userId = request.user.userId;
        const payload = evidenceConfirmSchema.parse(request.body);
    ...
    server.post('/disputes/:disputeId/resolve', { preHandler: [server.authenticate] }, async (request, reply) => {
        const { disputeId } = request.params;
    ...
    server.get('/disputes/:disputeId', { preHandler: [server.authenticate] }, async (request, reply) => {
        const { disputeId } = request.params;
        const userId = request.user.userId;
        const dispute = await disputeService.getDisputeById(userId, disputeId);
        return reply.send(dispute);
    });
    server.get('/disputes', { preHandler: [server.authenticate] }, async (request, reply) => {
        const userId = request.user.userId;
