...
export const storagePlugin = fp(async (server) => {
    ...
    async function expireListings() {
        try {
            const { rowCount } = await server.db.query(`UPDATE public.listings SET status = 'EXPIRED', updated_at = NOW() WHERE status = 'ACTIVE' AND expires_at <= NOW()`);
            if (rowCount > 0) {
