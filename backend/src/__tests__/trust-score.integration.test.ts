...
import { TrustScoreCalculationRequest, TrustScoreMetrics } from '../types/trust-score';
...
describe('Trust Score Integration Tests', () => {
  ...
  describe('Score Persistence', () => {
    ...
    it('should record trust score history on change', async () => {
      ...
      const historyInsertCall = queryCalls.find((call: any) =>
        call[0].includes('INSERT INTO vault.farmer_trust_score_history'),
      );
  ...
  describe('Caching', () => {
    ...
    it('should re-cache after recalculation', async () => {
      ...
      const trustScoreCacheCall = setCalls.find((call: any) => call[0].includes(`trust_score:${farmerId}`));
  ...
  describe('Metric Aggregation', () => {
    ...
    it('should update metrics correctly on recalculation', async () => {
      ...
      const previousMetrics: TrustScoreMetrics = {
      ...
      const newMetrics: TrustScoreMetrics = {
        completedOrdersCount: 50,
        totalOrdersCount: 50,
        averageRating: 4.5,
        totalReviewsCount: 50,
  ...
  describe('Change Tracking', () => {
    it('should track score changes over time', async () => {
      ...
      const metrics1: TrustScoreMetrics = {
        completedOrdersCount: 10,
        totalOrdersCount: 10,
        averageRating: 4.0,
        totalReviewsCount: 10,
      ...
      const metrics2: TrustScoreMetrics = {
        ...metrics1,
        completedOrdersCount: 50,
        totalOrdersCount: 50,
        averageRating: 4.8,
  ...
  describe('Database Operations', () => {
    it('should use ON CONFLICT for upsert', async () => {
      ...
      const upsertCall = queryCalls.find((call: any) =>
        call[0].includes('ON CONFLICT') && call[0].includes('farmer_trust_scores'),
      );
    ...
    it('should store components separately', async () => {
      ...
      const queryCalls = mockDb.query.mock.calls;
      const componentCalls = queryCalls.filter((call: any) =>
        call[0].includes('farmer_trust_score_components'),
      );
