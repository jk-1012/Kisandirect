...
describe('Trust Score Service', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    trustScoreService = createTrustScoreService(mockServer as any);
  });
  ...
  describe('Score Calculation', () => {
    ...
    it('should apply KYC bonuses correctly', () => {
      ...
      // Test each KYC level
      const kycTests = [
        { level: 'NONE' as const, expectedBonus: 0 },
        { level: 'BASIC' as const, expectedBonus: config.bonuses.kycBasic },
        { level: 'VERIFIED' as const, expectedBonus: config.bonuses.kycVerified },
        { level: 'ADVANCED' as const, expectedBonus: config.bonuses.kycAdvanced },
      ];
    ...
    it('should apply dispute penalties correctly', () => {
      ...
      const expectedPenalty = Math.min(
        5 * config.penalties.disputePerUnresolved,
    ...
    it('should apply cancellation penalties correctly', () => {
      ...
      const breakdown = trustScoreService.calculateTrustScore(metrics, config);
      const expectedPenalty = Math.min(
        20 * config.penalties.cancellationPerCancellation,
    ...
    it('should cap score between 0 and 100', () => {
      ...
      const metricsHigh: TrustScoreMetrics = {
      ...
      const metricsLow: TrustScoreMetrics = {
        completedOrdersCount: 0,
        totalOrdersCount: 100,
        averageRating: 1.0,
    ...
    it('should categorize scores correctly', () => {
      ...
      for (const test of testCases) {
        ...
        const category =
          actualScore >= config.thresholds.excellentThreshold
            ? 'EXCELLENT'
  ...
  describe('Recommendations Generation', () => {
    ...
    it('should include dispute resolution recommendations', () => {
      ...
      expect(recommendations.some((r: string) => r.includes('dispute'))).toBe(true);
    });
    ...
    it('should include KYC upgrade recommendations', () => {
      ...
      expect(recommendations.some((r: string) => r.includes('KYC'))).toBe(true);
    });
  });
