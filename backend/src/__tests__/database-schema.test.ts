...
describe('Database schema repair validation', () => {
  ...
  test('price_alerts table uses farmer_id and threshold_price_per_kg_inr', async () => {
    const res = await client.query(
      `SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'price_alerts'`
    )
    const columns = res.rows.map((row: any) => row.column_name)
    expect(columns).toEqual(expect.arrayContaining([
      'farmer_id',
  ...
  test('missing SRS tables were created', async () => {
    ...
    const existing = res.rows.map((row: any) => row.table_name)
  ...
  test('primary lookup indexes exist for listings and orders', async () => {
    ...
    const indexes = res.rows.map((row: any) => row.indexname)
