...
function getCdnBase(server: any) {
...
function findCategoryForCrop(cropType: string) {
...
function labelTriggersModeration(label: string) {
...
export const queuePlugin = fp(async (server) => {
  ...
  new Worker(
    'bulk-register',
    async (job) => {
      const rows = job.data.rows as Array<Record<string, string>>;
      const total = rows.length;
      let processed = 0;
      let failed = 0;
      const failedMobiles: string[] = [];
        ...
        try {
          ...
          if (existing.rows.length === 0) {
            await server.db.query(
              'INSERT INTO public.users (phone, role, language, kyc_status, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW())',
              [row.mobile, 'FARMER', 'en', 'PENDING_KYC']
            );
          }
  ...
  new Worker(
    'photo-processing',
    async (job: Job) => {
      const { userId, s3Key, bucketName } = job.data as { userId: string; s3Key: string; bucketName: string };
      ...
      await server.storage.s3Client.send(
        new PutObjectCommand({ Bucket: bucketName, Key: thumbnailKey, Body: thumbnailBuffer, ContentType: 'image/webp', ACL: 'public-read' })
      );
      await server.storage.s3Client.send(
        new PutObjectCommand({ Bucket: bucketName, Key: mediumKey, Body: mediumBuffer, ContentType: 'image/webp', ACL: 'public-read' })
      );
      await server.storage.s3Client.send(new DeleteObjectCommand({ Bucket: bucketName, Key: s3Key }));

      const apiKey = process.env.GOOGLE_CLOUD_VISION_API_KEY;
      let aiDetectedCrop: string | null = null;
      ...
      try {
        ...
        const labels = json.responses?.[0]?.labelAnnotations as Array<{ description?: string; score?: number }> | undefined;
        const safeSearch = json.responses?.[0]?.safeSearchAnnotation as Record<string, string> | undefined;
  ...
  new Worker(
    ...
    async (job: Job) => {
      ...
      if (job.name === 'LISTING_EXPIRE') {
        await server.db.query(
          `UPDATE public.listings SET status = 'EXPIRED', updated_at = NOW() WHERE listing_id = $1 AND status = 'ACTIVE'`,
          [listingId]
        );
  ...
  new Worker(
    'sitemap',
    async (job: Job) => {
      if (job.name !== 'GENERATE_SITEMAP') return null;
      server.log.info({ jobId: job.id }, 'generating catalogue sitemap');
      const rows = await server.db.query(
        `SELECT DISTINCT fp.state_code AS state, l.crop_type
         FROM public.listings l
         JOIN public.farmer_profiles fp ON l.farmer_id = fp.user_id
         WHERE l.status = 'ACTIVE'`
      );
      const combos = rows.rows.filter((r: any) => r.state && r.crop_type).map((r: any) => ({ state: String(r.state).toLowerCase(), crop: String(r.crop_type).toLowerCase() }));
      const urls = combos.map((c: any) => `<url><loc>https://kisandirect.in/buy/${encodeURIComponent(c.state)}/${encodeURIComponent(c.crop)}</loc></url>`).join('\n');
