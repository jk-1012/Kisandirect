...
const mockDb = {
  query: jest.fn() as jest.MockedFunction<(...args: any[]) => Promise<any>>
};
...
const mockServer: any = {
  ...
  httpErrors: {
    ...
    tooManyRequests: (msg: string) => new Error(msg),
...
describe('AuthService', () => {
  ...
  describe('JWT generation and refresh token rotation', () => {
    it('creates user and returns tokens', async () => {
      ...
      const compareMock = jest.spyOn(bcrypt, 'compare') as unknown as { mockResolvedValue: (value: boolean) => void };
