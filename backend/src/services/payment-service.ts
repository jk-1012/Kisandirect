...
export function createPaymentService(server: FastifyInstance) {
  /**
  ...
  async function createOrder(payload: {
    ...
    const listingRes = await server.db.query(
      'SELECT * FROM public.listings WHERE id = $1 AND status = $2',
      [payload.listing_id, 'ACTIVE']
    );
   ...
   */
  async function processPaymentWebhook(payload: any, webhookSignature: string) {
    ...
    if (event === 'payment.authorized') {
      ...
      const orderRes = await server.db.query(
        `UPDATE public.orders
         ...
         RETURNING *`,
        ['CONFIRMED', payment.id, 'CONFIRMED', payment.order_id]
      );
      ...
      server.log.info(
        { order_id: order.order_id, payment_id: payment.id, amount_inr: payment.amount / 100 },
        'Payment confirmed, order status updated'
      );

      // Queue e-challan generation
      try {
        await server.queues.listingQueue.add(
          'GENERATE_CHALLAN',
          { order_id: order.id },
    ...
    if (event === 'payment.failed') {
      ...
      await server.db.query(
        `UPDATE public.orders
         SET payment_status = $1, razorpay_payment_id = $2, status = $3, updated_at = NOW()
         WHERE razorpay_order_id = $4`,
        ['FAILED', payment.id, 'PAYMENT_FAILED', payment.order_id]
      );
   ...
   */
  async function confirmDelivery(buyerId: string, orderId: string, rating?: number) {
    const orderRes = await server.db.query(
      `SELECT * FROM public.orders WHERE order_id = $1 AND buyer_id = $2`,
      [orderId, buyerId]
    );
    ...
    try {
      ...
      await server.db.query(
        `UPDATE public.orders
         ...
         WHERE order_id = $3`,
        ['COMPLETED', deliveryDate, orderId]
      );

      // Store buyer rating if provided
      if (rating && rating >= 1 && rating <= 5) {
        await server.db.query(
          `INSERT INTO public.order_ratings (order_id, buyer_id, farmer_id, rating, created_at)
           VALUES ($1, $2, $3, $4, NOW())
           ON CONFLICT (order_id) DO UPDATE SET rating = EXCLUDED.rating`,
          [order.id, buyerId, order.farmer_id, rating]
        );
      }
      ...
      try {
        await server.queues.marketQueue.add(
          'UPDATE_TRUST_SCORE',
          { farmer_id: order.farmer_id },
      ...
      server.log.info(
        { order_id: orderId, farmer_id: order.farmer_id, rating },
  ...
  async function getOrderPaymentStatus(orderId: string) {
    const result = await server.db.query(
      `SELECT order_id, status, payment_status, razorpay_order_id, razorpay_payment_id, 
   ...
   * Get buyer's orders
   */
  async function getBuyerOrders(buyerId: string, opts: { status?: string; limit?: number }) {
