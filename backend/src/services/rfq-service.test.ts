...
describe('rfq-service', () => {
  it('creates an RFQ and notifies matched farmers', async () => {
    ...
    const fakeServer: any = {
      db: {
        query: jest.fn((text: string, values: any[]) => {
          ...
          if (text.includes('SELECT id, is_institutional')) return Promise.resolve({ rows: [{ id: 'buyer-1', is_institutional: true }] });
          if (text.includes('INSERT INTO public.rfqs')) return Promise.resolve({ rows: [{ id: 123 }] });
          if (text.includes('SELECT u.id')) return Promise.resolve({ rows: [{ id: 'farmer-1', phone: '9999999999' }] });
    ...
    const svc = createRfqService(fakeServer as any);
  ...
  it('allows farmer to submit a quote and notifies buyer', async () => {
    const queries: Array<{ text: string; values: any[] }> = [];
    const fakeServer: any = {
      db: { query: jest.fn((text: string, values: any[]) => { queries.push({ text, values }); if (text.includes('SELECT * FROM public.rfqs')) return Promise.resolve({ rows: [{ id: 10, rfq_id: 'RFQ-1', buyer_id: 'buyer-1', expires_at: new Date(Date.now() + 10000).toISOString() }] }); if (text.includes('SELECT id FROM public.users WHERE id')) return Promise.resolve({ rows: [{ id: 'farmer-1' }] }); if (text.includes('INSERT INTO public.rfq_quotes')) return Promise.resolve({ rows: [{ id: 1 }] }); if (text.includes('SELECT phone FROM public.users WHERE id =')) return Promise.resolve({ rows: [{ phone: '9999999999' }] }); return Promise.resolve({ rows: [] }); }) },
      log: { warn: jest.fn() }
    };
