...
export function createCommissionEngine(server: FastifyInstance) {
  /**
  ...
  async function getCommissionConfig(farmerId: string): Promise<CommissionConfig> {
    ...
    const farmerRes = await server.db.query(
      `SELECT u.trust_score, fp.annual_payout_inr
       FROM public.users u
    ...
    const configRes = await server.db.query(
      `SELECT * FROM vault.commission_config
    ...
    const configs = configRes.rows as CommissionConfig[];
    ...
    const defaultRes = await server.db.query(
      `SELECT * FROM vault.commission_config WHERE commission_name = 'STANDARD'`,
      []
    );
  ...
  async function calculateOrderCommission(
    ...
    const commissionPaise = calculateCommissionPrecise(
      subtotalPaise,
...
export type CommissionEngine = ReturnType<typeof createCommissionEngine>;

// ===========================
// SETTLEMENT ENGINE
// ===========================

export function createSettlementEngine(server: FastifyInstance) {
  const commissionEngine = createCommissionEngine(server);

  /**
   * Get previous ledger entry hash for chain verification
   */
  async function getPreviousLedgerHash(): Promise<string> {
    const result = await server.db.query(
      `SELECT ledger_hash FROM vault.payment_ledger ORDER BY created_at DESC LIMIT 1`,
      []
    );
  ...
  async function createLedgerEntry(
    ...
    const lastEntry = await server.db.query(
      `SELECT running_balance_paise FROM vault.payment_ledger ORDER BY created_at DESC LIMIT 1`,
      []
    );
   ...
   */
  async function verifyLedgerChain(): Promise<{ verified: boolean; brokenAt?: string }> {
    const result = await server.db.query(
      'SELECT verify_ledger_chain() as result',
      []
    );
  ...
  async function calculateSettlement(
    ...
    const entriesResult = await server.db.query(
    ...
    const entries = entriesResult.rows as PaymentLedgerEntry[];
    ...
    // Get order counts
    const countsResult = await server.db.query(
      `SELECT 
   ...
   */
  async function verifySettlement(settlementId: string): Promise<boolean> {
    const settlement = await server.db.query(
      `SELECT * FROM vault.settlements WHERE settlement_id = $1`,
      [settlementId]
    );
    ...
    const recalc = await calculateSettlement(
      sett.farmer_id,
      sett.settlement_period_start,
      sett.settlement_period_end
    );

    // Verify amounts match exactly
    const amountsMatch = 
      sett.total_sales_paise === recalc.total_sales_paise &&
