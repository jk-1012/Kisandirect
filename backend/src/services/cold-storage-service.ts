...
export function createColdStorageService(server: FastifyInstance) {
  async function searchColdStoragesNearby(
    lat: number,
    lng: number,
    radiusKm = 50,
    capacityNeededMt?: number,
    cropType?: string
  ) {
    const radiusMeters = Math.min(Math.max(radiusKm, 1), 200) * 1000;
    const values: Array<number | string | boolean> = [lng, lat, radiusMeters];
    ...
    const result = await server.db.query(
      `SELECT id, nabard_id, name, operator_user_id, address, state_code, district, geo_lat, geo_lng,
    ...
    return result.rows.map((row: any) => {
      ...
      return {
        ...
        availability_status:
          availableRatio > 0.2 ? 'AVAILABLE' : availableRatio > 0 ? 'LIMITED' : 'FULL'
      };
    });
  }

  async function getFacilityAvailability(facilityId: string, fromDate: string, toDate: string) {
    ...
    const facilityRes = await server.db.query(
      `SELECT id, nabard_id, name, state_code, district, geo_lat, geo_lng, total_capacity_mt, available_capacity_mt
       FROM public.cold_storage_facilities
       WHERE id = $1`,
      [facilityId]
    );
    ...
    const bookingsRes = await server.db.query(
      `SELECT booking_ref, farmer_id, entry_date, exit_date, quantity_mt, crop_type, temperature_agreed_min,
    ...
    return {
      ...
      from_date: fromDate,
      to_date: toDate,
      bookings: bookingsRes.rows.map((row: any) => ({
  ...
  async function createBooking(
    ...
    quantityMt: number,
    cropType: string,
    temperatureAgreedMin?: number,
    temperatureAgreedMax?: number
  ) {
    if (quantityMt <= 0) {
    ...
    try {
      const facilityRes = await server.db.query(
        `SELECT id, name, total_capacity_mt, available_capacity_mt, price_per_mt_per_week_paise,
      ...
      const conflictRes = await server.db.query(
        `SELECT id FROM public.cold_storage_bookings
         WHERE facility_id = $1
           AND farmer_id = $2
           AND status NOT IN ('CANCELLED', 'COMPLETED')
           AND (entry_date, exit_date) OVERLAPS ($3::date, $4::date)
         LIMIT 1`,
        [facilityId, farmerId, entryDate, exitDate]
      );
      if (conflictRes.rowCount > 0) {
        throw server.httpErrors.conflict('You already have an overlapping booking for this facility');
      }

      const bookedRes = await server.db.query(
        `SELECT COALESCE(SUM(quantity_mt), 0)::numeric AS booked_mt
         FROM public.cold_storage_bookings
         WHERE facility_id = $1
           AND status NOT IN ('CANCELLED', 'COMPLETED')
           AND (entry_date, exit_date) OVERLAPS ($2::date, $3::date)`,
        [facilityId, entryDate, exitDate]
      );
      ...
      const bookingRes = await server.db.query(
        `INSERT INTO public.cold_storage_bookings (
  ...
  async function getUserBookings(farmerId: string) {
    const result = await server.db.query(
      `SELECT b.booking_ref, b.status, b.entry_date, b.exit_date, b.quantity_mt, b.crop_type,
