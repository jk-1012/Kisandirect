...
export default async function (server: FastifyInstance) {
  ...
  server.get('/admin/tds/summary', { preHandler: [server.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
    if (request.user.role !== 'ADMIN') {
      return reply.code(403).send({ error: 'Forbidden' });
    }

    const query = summaryQuerySchema.parse(request.query as Record<string, unknown>);
  ...
  server.get('/farmers/me/tds/form16a', { preHandler: [server.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
    if (request.user.role !== 'FARMER') {
      return reply.code(403).send({ error: 'Forbidden' });
    }

    const query = form16QuerySchema.parse(request.query as Record<string, unknown>);
