...
export default async function (server: FastifyInstance) {
  ...
  const challanConfig: EChallanConfig = {
    otpLength: 6,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const orderCheck = await db.query(
          'SELECT id, farmer_id, buyer_id FROM public.orders WHERE id = $1',
          [orderId],
        );
  ...
  server.post<{ Params: { id: string }; Body: any }>(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const enrichedRequest: SignChallanRequest = {
          challanId,
          ...body,
          deviceInfo: {
            ...body.deviceInfo,
        ...
        server.log.info(
          `Signature captured for challan ${challanId} by ${body.signerType} from ${requestIp}`,
        );
   ...
   */
  server.get<{ Params: { id: string }; Querystring: any }>(
    '/orders/:id/challan/download',
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const challanId = request.params.id;
