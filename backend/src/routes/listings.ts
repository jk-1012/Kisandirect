...
export default async function (server: FastifyInstance) {
  ...
  server.post('/listings/photo/upload-url', { preHandler: [server.authenticate, server.requireKYC] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const query = photoUploadUrlSchema.parse(request.query as Record<string, unknown>);
    const response = await listingService.getPhotoUploadUrl(request.user.userId, query.filename, query.content_type);
    return reply.send(response);
  });

  server.post('/listings/photo/process', { preHandler: server.authenticate }, async (request: FastifyRequest, reply: FastifyReply) => {
    const body = photoProcessSchema.parse(request.body as Record<string, unknown>);
  ...
  server.get('/listings/mine', { preHandler: [server.authenticate, server.requireKYC] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const listings = await listingService.getMyListings(request.user.userId);
    return reply.send({ listings });
  });

  server.get('/listings/:listingId', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post('/listings/:listingId/cancel', { preHandler: [server.authenticate, server.requireKYC] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { listingId } = request.params as { listingId: string };
    const result = await listingService.cancelListing(request.user.userId, listingId);
    return reply.send(result);
  });

  server.patch('/listings/:listingId', { preHandler: [server.authenticate, server.requireKYC] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { listingId } = request.params as { listingId: string };
    const payload = listingUpdateSchema.parse(request.body as Record<string, unknown>);
    const result = await listingService.updateListing(request.user.userId, listingId, payload);
    return reply.send(result);
  });

  server.delete('/listings/:listingId', { preHandler: [server.authenticate, server.requireKYC] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { listingId } = request.params as { listingId: string };
    const result = await listingService.deleteListing(request.user.userId, listingId);
    return reply.send(result);
  });

  server.get('/listings/search', async (request: FastifyRequest, reply: FastifyReply) => {
    const query = searchSchema.parse(request.query as Record<string, unknown>);
