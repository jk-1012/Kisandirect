...
export default async function (server: FastifyInstance) {
  ...
  server.post('/payments/webhook', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const verifiedWebhook = (request as any).verifiedWebhook;
      const webhookEvent = (request as any).webhookEvent;
      ...
      server.log.info(
        { webhook_event_id: event_id, event_type },
      ...
      try {
        ...
        await webhookService.failWebhookProcessing(event_id, (err as Error).message);
        throw err;
      }
    } catch (err) {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const orderRes = await server.db.query(
          `SELECT o.*, ea.escrow_id FROM public.orders o
        ...
        server.log.info(
          { order_id: orderId, escrow_id: escrowId },
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { orderId } = request.params as { orderId: string };
        const payload = refundRequestSchema.parse(request.body as Record<string, unknown>);
        const userId = request.user.userId;
   ...
   * Public endpoint
   */
  server.get('/orders/:orderId/payment-status', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { orderId } = request.params as { orderId: string };

      const result = await server.db.query(
        `SELECT o.*, ea.escrow_id, ea.escrow_status, ea.total_amount_paise, ea.commission_paise
