 ...
 */
export async function registerTrustScoreRoutes(
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const { detailed, includeHistory } = request.query as { detailed?: string; includeHistory?: string };
      ...
      try {
        ...
        const percentile = Math.round(
          (summaryStats.distribution.poor +
   ...
   */
  server.get('/farmers/:farmerId/trust-score/percentiles', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const summaryStats = await trustScoreService.getTrustScoreSummaryStats();
   ...
   */
  server.post(
    '/admin/trust-score/recalculate/:farmerId',
    {
      schema: {
        params: z.object({
          farmerId: z.string().uuid('Invalid farmer ID'),
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const result = await queueTrustScoreRecalculation(
          server,
          farmerId,
   ...
   */
  server.post(
    '/admin/trust-score/recalculate-batch',
    {
      schema: {
        body: z.object({
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const { trustScoreBelow, trustScoreAbove, inactiveForDays, kycLevel, batchSize, priority } =
          request.body as any;

        // Queue batch recalculation as a job
        const queue = server.queues.payoutQueue;
        await queue.add(
          'BATCH_RECALCULATE_TRUST_SCORES',
          {
            filters: {
              trustScoreBelow,
              trustScoreAbove,
...
export type TrustScoreRoutes = typeof registerTrustScoreRoutes;
