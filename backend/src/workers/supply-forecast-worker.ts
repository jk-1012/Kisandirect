...
export interface SupplyForecastJob {
  cropType: string;
  region?: string;
  forecastDays?: number;
}
...
export async function supplyForecastHandler(
  job: Job<SupplyForecastJob>,
  ...
  try {
    ...
    if (!supplyResult.rows || supplyResult.rows.length === 0) {
      server.log.info(
        { jobId: job.id, cropType, region },
        'No supply data available for forecast',
      );
    ...
    if (dataPoints >= 2) {
      ...
      const quantityChange =
        ((latestData.total_quantity - oldest.total_quantity) / oldest.total_quantity) * 100;
    ...
    const forecastResult = await server.db.query(
      `INSERT INTO supply_forecasts 
       (crop_type, region, forecast_date, supply_trend, avg_daily_volume, data_points, created_at)
       VALUES ($1, $2, NOW(), $3, $4, $5, NOW())
       RETURNING id, forecast_date`,
      [cropType, region || null, supplyTrend, latestData.total_quantity, dataPoints],
    );
