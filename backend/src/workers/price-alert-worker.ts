...
export interface PriceAlertJob {
  alertId: string;
  userId: string;
  cropType: string;
  targetPrice: number;
  operator: 'ABOVE' | 'BELOW'; // ABOVE = price goes above, BELOW = price goes below
  market?: string;
}
...
export async function priceAlertHandler(
  job: Job<PriceAlertJob>,
    ...
    if (triggered) {
      ...
      try {
        ...
        if (notificationQueue) {
          ...
          server.log.info(
            { jobId: job.id, alertId, userId },
            'Price alert notification queued',
          );
        }
      } catch (notificationError: any) {
        server.log.warn(
          { jobId: job.id, alertId, error: notificationError?.message },
          'Failed to queue price alert notification',
        );
      ...
      try {
        await server.db.query(
          `INSERT INTO price_alert_triggers (alert_id, user_id, triggered_at, current_price) 
           VALUES ($1, $2, NOW(), $3)`,
          [alertId, userId, currentPrice],
        );
      } catch (recordError: any) {
        server.log.warn(
          { jobId: job.id, alertId, error: recordError?.message },
          'Failed to record price alert trigger',
        );
      }
    }
