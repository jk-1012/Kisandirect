...
export interface SitemapGeneratorJob {
  sitemapType: 'LISTINGS' | 'STOREFRONTS' | 'PAGES';
  region?: string;
}
...
export async function sitemapGeneratorHandler(
  job: Job<SitemapGeneratorJob>,
  ...
  try {
    ...
    if (urls.length === 0) {
      server.log.warn(
        { jobId: job.id, sitemapType, region },
        'No URLs to generate sitemap',
      );
    ...
    try {
      await server.db.query(
        `INSERT INTO sitemaps (sitemap_type, region, url_count, file_path, published_at, created_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())`,
        [sitemapType, region || null, urls.length, s3Path],
      );
    } catch (dbError: any) {
...
async function generateListingsSitemap(
  ...
  let query =
    'SELECT id, slug, updated_at FROM listings WHERE status = $1 ORDER BY updated_at DESC LIMIT 50000';
 ...
 */
async function generateStorefrontsSitemap(
  server: FastifyInstance,
): Promise<Array<{ loc: string; lastmod?: string; changefreq?: string; priority?: number }>> {
  const baseUrl = process.env.APP_URL || 'https://kisandirect.com';

  const result = await server.db.query(
    'SELECT id, slug, updated_at FROM farmer_storefronts WHERE is_published = true ORDER BY updated_at DESC LIMIT 50000',
  );
