...
export async function processReleaseEscrow(server: FastifyInstance, jobData: { orderId: string }) {
  ...
  await server.db.query(
    ...
    [Math.round(subtotalPaise / 100), Math.round(tds_paise / 100), order.farmer_id]
  );
  ...
  await server.db.query(
    ...
    [`TXN-${crypto.randomBytes(6).toString('hex')}`, order.id, 'ESCROW_RELEASED', payoutPaise, order.farmer_id, order.buyer_id, { payout_id: payoutId, tds_paise }, prevHash, entryHash]
  );
