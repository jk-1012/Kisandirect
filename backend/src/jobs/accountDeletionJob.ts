...
export async function executeAccountDeletion(server: FastifyInstance, jobData: { userId: string }) {
  ...
  try {
    ...
    await server.db.query(
      `INSERT INTO public.consent_records (user_id, consent_type, consented, policy_version, consented_at)
       VALUES ($1, 'ACCOUNT_DELETED', FALSE, 'N/A', NOW())`,
      [userId]
    );
