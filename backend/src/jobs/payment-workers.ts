...
export async function processEscrowRelease(
  ...
  try {
    // Get escrow account
    const escrowRes = await server.db.query(
      `SELECT ea.* FROM vault.escrow_accounts ea WHERE ea.escrow_id = $1`,
      [escrow_id]
    );
    ...
    if (escrow.escrow_status !== 'HELD') {
      server.log.warn(
        { escrow_id, status: escrow.escrow_status },
        'Escrow not in HELD status, skipping release'
      );
    ...
    if (order.order_status !== 'DELIVERED') {
      server.log.warn(
        { order_id, status: order.order_status },
        'Order not delivered, cannot release escrow'
      );
    ...
    const payoutResult = await initiateRazorpayPayout(
      ...
      farmer_id,
      payoutAmount,
      `Escrow release for order ${order_id}`,
      razorpay_order_id
    );

    if (!payoutResult.success) {
      server.log.error(
        { escrow_id, farmer_id, error: payoutResult.error },
        'Payout initiation failed'
      );

      // Create failed payout record for retry
      await createFailedPayoutRecord(
        server,
        farmer_id,
        order_id,
        payoutAmount,
  ...
  try {
    ...
    await server.db.query(
      `UPDATE vault.escrow_accounts
       ...
       WHERE escrow_id = $3`,
      [refund_amount_paise || escrow.total_amount_paise, reason, escrow_id]
    );
    ...
    if (escrow.razorpay_payment_id) {
      ...
      if (!refundResult.success) {
        server.log.error(
          { escrow_id, payment_id: escrow.razorpay_payment_id },
    ...
    server.log.info(
      { escrow_id, refund_amount_paise: refund_amount_paise || escrow.total_amount_paise },
...
export async function processPayoutReconciliation(
  server: FastifyInstance,
  jobData: {
    reconciliation_date?: string; // YYYY-MM-DD, defaults to today
  } = {}
) {
  ...
  try {
    ...
    const dbPayouts = await server.db.query(
      `SELECT rz_payout_id, amount_paise FROM vault.payment_ledger
       WHERE txn_type = 'ESCROW_RELEASED' AND DATE(created_at) = $1`,
      [reconciliationDate]
    );

    const dbPayoutMap = new Map(
      (dbPayouts.rows as Array<{ rz_payout_id: string; amount_paise: number }>).map(r => [r.rz_payout_id, r.amount_paise])
    );
    ...
    await server.db.query(
      `INSERT INTO vault.payment_reconciliation
...
export async function processFailedPayoutRetry(
  server: FastifyInstance,
  jobData: {
    failed_payout_id: number;
  }
) {
  ...
  try {
    ...
    const fpRes = await server.db.query(
      `SELECT * FROM vault.failed_payouts WHERE id = $1`,
      [failed_payout_id]
    );
    ...
    if (fp.retry_count >= fp.max_retries) {
      await server.db.query(
        `UPDATE vault.failed_payouts SET status = $1 WHERE id = $2`,
        ['MANUAL_REVIEW', failed_payout_id]
      );

      server.log.warn(
        { failed_payout_id, retry_count: fp.retry_count },
        'Max retries exceeded, marking for manual review'
      );
    ...
    if (payoutResult.success) {
      ...
      await server.db.query(
        `UPDATE vault.failed_payouts 
         ...
         WHERE id = $3`,
        ['SUCCEEDED', payoutResult.payout_id, failed_payout_id]
      );

      server.log.info(
        { failed_payout_id, new_payout_id: payoutResult.payout_id },
        'Failed payout retry succeeded'
      );
      ...
      await server.db.query(
        `UPDATE vault.failed_payouts 
         ...
         WHERE id = $3`,
        [nextRetryTime.toISOString(), payoutResult.error, failed_payout_id]
      );

      server.log.info(
        { failed_payout_id, retry_count: fp.retry_count + 1, next_retry_at: nextRetryTime },
        'Failed payout retry scheduled'
      );
...
): Promise<{ success: boolean; note: string; webhook_event_id: string }> {
  const { webhook_event_id, error, payload, timestamp } = jobData;

  server.log.error(
    { webhook_event_id, error, timestamp },
    'Processing webhook from dead-letter queue for manual review'
  );
...
async function initiateRazorpayPayout(
  server: FastifyInstance,
  farmerId: string,
  amountPaise: number,
  narration: string,
  ...
  try {
    ...
    const kycRes = await server.db.query(
      `SELECT fund_account_id FROM vault.farmer_kyc WHERE farmer_id = $1`,
      [farmerId]
    );
...
async function fetchRazorpayPayouts(
  server: FastifyInstance,
  date: string
): Promise<Array<{ id: string; amount: number }>> {
  try {
    const response = await fetch(
      `https://api.razorpay.com/v1/payouts?from=${date}&to=${date}&limit=100`,
      {
        headers: {
          'Authorization': `Basic ${Buffer.from(
            `${process.env.RAZORPAY_KEY_ID}:${process.env.RAZORPAY_KEY_SECRET}`
    ...
    const data = (await response.json()) as any;
    return (data.items || []).map((p: any) => ({ id: p.id, amount: p.amount }));
  } catch (err) {
...
async function createFailedPayoutRecord(
  server: FastifyInstance,
  farmerId: string,
  orderId: string,
  amountPaise: number,
